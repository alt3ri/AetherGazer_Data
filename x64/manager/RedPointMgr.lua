...
function slot0.Init(slot0)
	...
	slot6 = RedPointConst.RESOURSE_STAGE
	...
	slot9 = RedPointConst.CHALLENGE_DEMO
	...
	for slot13, slot14 in pairs(HeroCfg.get_id_list_by_private[0]) do
		...
		slot36 = RedPointConst.HERO_ARCHIVE_ID .. slot14

		slot0:addGroup(slot36, {
	...
	slot27 = ActivityConst.ACTIVITY_3_0_SEVEN_DAY_SKIN_SIGN

	slot0:addGroup(string.format("%s_%s", RedPointConst.SEVEN_DAY_SIGN_SKIN, slot27), {
		string.format("%s_%s", RedPointConst.ACTIVITY_3_0_SURPRISE_GIFT, ActivityConst.ACTIVITY_3_0_SURPRISE_GIFT_DRAW)
	})

	slot20 = {}
	slot26 = ChipConst.TYPE_MANAGER

	for slot26, slot27 in ipairs(ChipCfg.get_id_list_by_type_id[slot26]) do
		table.insert({}, string.format("%s_%s", RedPointConst.CHIP_MANAGER, slot27))
		table.insert({}, string.format("%s_%s", RedPointConst.CHIP_MANAGER, slot27))
	end

	slot26 = ChipConst.TYPE_CHIP
	...
	slot26 = RedPointConst.CHIP_MANAGER
	slot27 = slot22

	slot0:addGroup(slot26, slot27)
	...
	slot28 = RedPointConst.FRIEND_FRIEND_REQUESTS
	...
	slot23 = {}
	slot27 = ItemConst.ITEM_TYPE.SCENE

	for slot27, slot28 in ipairs(ItemCfg.get_id_list_by_type[slot27]) do
	...
	slot29 = RedPointConst.MAIN_HOME_SDK

	slot0:addGroup(RedPointConst.MAIN_HOME_MENU, slot28)
	...
	slot26 = {}
	slot30 = ItemConst.ITEM_TYPE.STICKER_BG

	for slot30, slot31 in ipairs(ItemCfg.get_id_list_by_type[slot30]) do
	...
	slot34 = RedPointConst.ILLU_HERO
	...
	slot33 = {}
	slot38 = ItemConst.ITEM_TYPE.CHAT_BUBBLE

	for slot38, slot39 in ipairs(ItemCfg.get_id_list_by_type[slot38]) do
	...
	slot40 = RedPointConst.NOOB_BP_RECHARGE

	slot0:addGroup(RedPointConst.NEWBIE_POINT, slot39)
		...
		if type(NoobVersionCfg[slot40].noob_advance_task_type) == "table" then
			for slot46, slot47 in ipairs(slot42) do
				...
				for slot55, slot56 in ipairs(NoobAdvanceTaskPhaseListCfg[slot41.noob_advance_task_phase[slot46]].phase_list) do
					slot62 = slot55
					slot51[#slot51 + 1] = string.format("%s_%d_%d", RedPointConst.NOOB_ADVANCE, slot47, slot62)
					slot58 = {}
	...
	slot41 = RedPointConst.MATRIX_TERMINAL_GIFT_SHELTER

	slot0:addGroup(RedPointConst.MATRIX_TERMINAL_GIFT, slot40)
	...
	slot37 = {}
	slot42 = "OSIRIS_DEMON"
	slot41 = BattleConst.STAGE_TYPE_NEW[slot42]

	for slot41, slot42 in ipairs(StageGroupCfg.get_id_list_by_type[slot41]) do
		table.insert(slot37, DemonChallengeData:GetRedPointConst(slot41))
	end

	slot42 = slot37

	slot0:addGroup(RedPointConst.OSIRIS_DEMON, slot42)

	slot41 = "get_id_list_by_activity_id"

	for slot41, slot42 in pairs(ActivityHeroTrialCfg[slot41]) do
		slot43 = {}
	...
	slot42 = "SKIN_TRIAL"
	slot41 = ActivityTemplateConst[slot42]

	for slot41, slot42 in ipairs(ActivityCfg.get_id_list_by_activity_template[slot41]) do
		slot44 = {}
	...
	for slot43, slot44 in ipairs(ActivityCfg.get_id_list_by_activity_template[ActivityTemplateConst.PARKOUR] or {}) do
		...
		slot51 = RedPointConst.PARKOUR_ENTRUST_ALL
		slot50 = string.format

		slot0:addGroup(string.format("%s_%s", slot51, slot44), {
			string.format("%s_%s", RedPointConst.PARKOUR_REWARD, slot44),
			slot50("%s_%s", RedPointConst.PARKOUR_ENTRUST, slot44)
		})
	...
	for slot50, slot51 in ipairs(ActivityCfg.get_id_list_by_activity_template[ActivityTemplateConst.GUILD_ACTIVITY] or {}) do
		...
		slot0:addGroup(string.format("%s_%s", RedPointConst.GUILD_ACTIVITY, slot51), {
			...
			slot57("%s_%s", RedPointConst.GUILD_ACTIVITY_UNGET_COIN, slot51)
		})

		slot52 = {}
		slot56 = "get_id_list_by_activity_id_and_race"

		for slot56, slot57 in pairs(TalentTreeCfg[slot56][slot51]) do
			slot63 = slot56
			slot52[#slot52 + 1] = string.format("%s_%d_%d", RedPointConst.GUILD_ACTIVITY_AFFIX, slot51, slot63)
			slot59 = {}
	...
	for slot51, slot52 in pairs(GuildActivitySPTools:GetActivityState()) do
		...
		if TalentTreeCfg.get_id_list_by_activity_id_and_race[slot53] then
			slot58 = "get_id_list_by_activity_id_and_race"

			for slot58, slot59 in pairs(TalentTreeCfg[slot58][slot53]) do
				slot65 = slot58
				slot54[#slot54 + 1] = string.format("%s_%d_%d", RedPointConst.GUILD_ACTIVITY_SP_AFFIX, slot53, slot65)
				slot61 = {}
	...
	slot56 = RedPointConst.MARDUK_SPECIAL
	slot55 = string.format

	slot0:addGroup(string.format("%s_%d", slot56, ActivityConst.FACTORY_MARDUK), {
		string.format("%s_%d", RedPointConst.MARDUK_SPECIAL_NOT_FIGHT, ActivityConst.FACTORY_MARDUK),
		slot55("%s_%d", RedPointConst.MARDUK_SPECIAL_REWARD, ActivityConst.FACTORY_MARDUK)
	})
	...
	slot56 = 1
	slot60 = "FurnitureTypeNum"

	for slot60, slot61 in pairs(DormEnum[slot60]) do
		slot56 = slot56 + 1
	end
	...
	slot0:addGroup(RedPointConst.CANTEEN_UNLOCK_FOOD, slot61)
	...
	slot57 = {}
	slot61 = "FurnitureMainType"

	for slot61, slot62 in pairs(DormEnum[slot61]) do
	...
	slot63 = RedPointConst.DORM_PLACEMENT

	slot0:addGroup(RedPointConst.DORM, slot62)
	...
	slot0:addGroup(RedPointConst.DORM_PLACEMENT, slot63)
	...
	for slot66, slot67 in ipairs(DormSuitData:GetSuitIDListByType(DormSuitTools.DORM_SUIT_TYPE[slot66])) do
	...
	for slot68, slot69 in ipairs(DormSuitData:GetSuitIDListByType(DormSuitTools.DORM_SUIT_TYPE[slot68])) do
	...
	slot73 = "ABYSS_CHALLENGE"
	slot72 = string.format

	slot0:addGroup(RedPointConst.ABYSS, {
		RedPointConst.ABYSS_REWARD,
		RedPointConst[slot73],
		slot72(ServerRedPointPrefix, ServerRedPoint.DISORDER_TRAP_NEW_RED, "ROOT")
	})
	...
	for slot72, slot73 in pairs(MailSpecialLetterCfg.get_id_list_by_letter_belongs) do
		...
		table.insert(slot68, RedPointConst.LETTER_SENDER_ID .. slot78)
	...
	slot0:addGroup(RedPointConst.RECALL, slot76)
	...
	slot79 = RedPointConst.SETTING_REMIND

	slot0:addGroup(RedPointConst.SETTING_NEW_FUNCTION, slot78)
	...
	slot79 = "STRONGHOLD"
	slot78 = ActivityTemplateConst[slot79]

	for slot78, slot79 in ipairs(ActivityCfg.get_id_list_by_activity_template[slot78]) do
	...
	slot75 = {}
	slot80 = "ACTIVITY_SKIN_DRAW"
	slot79 = ActivityTemplateConst[slot80]

	for slot79, slot80 in ipairs(ActivityCfg.get_id_list_by_activity_template[slot79]) do
