...
function slot1.GetFreeItem(slot0)
	...
	if not slot0.gameObject_ then
		slot0.gameObject_ = Object.Instantiate(Asset.Load("Widget/Common/Pop/UIInquirepopup12"), manager.ui.uiMessage.transform)
...
function slot1.GetIsAllFree(slot0)
	for slot4, slot5 in pairs(slot0.itemList_) do
		if not slot5:IsFree() then
			return false
		end
	end
