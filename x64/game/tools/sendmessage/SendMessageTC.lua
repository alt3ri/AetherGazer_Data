...
return {
	typeToFunction = {
		...
		role_level = function (slot0)
			...
			if slot0.level == 16 or slot0.level == 21 or slot0.level == 30 then
				SDKTools.SetParamsAttribute("af_level_achieved" .. tostring(slot0.level), uv0)
			end
		end,
		task_finish = function (slot0)
			if slot0.taskId == 160011 then
				SDKTools.SetBaseAttribute("af_task_finish")
			end
		end,
		passStage = function (slot0)
			if slot0.passStage[1010305] then
