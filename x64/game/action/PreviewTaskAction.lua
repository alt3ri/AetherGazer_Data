...
manager.net:Bind(28023, function (slot0)
	if uv0 == false then
		uv0 = true
...
manager.net:Bind(28021, function (slot0)
...
return {
	TryToSubmitTask = function (slot0)
		manager.net:SendWithLoadingNew(28024, {
			id_list = {
				slot0
			}
		...
		for slot4, slot5 in pairs(PreviewTaskData:GetActivityIDList()) do
			...
			if ActivityData:GetActivityIsOpen(slot4) and AssignmentCfg.get_id_list_by_activity_id[slot4] ~= nil then
				for slot11, slot12 in ipairs(slot7) do
					if AssignmentCfg[slot12].need <= PreviewTaskData:GetTaskProgress(slot12) and not PreviewTaskData:GetTaskComplete(slot12) then
						slot6 = 1
