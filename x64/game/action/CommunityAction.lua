manager.net:Bind(31207, function (slot0)
	CommunityData:SetWelfareType(slot0.state)
	CommunityAction:RefreshRedPoint()
end)
manager.net:Bind(31209, function (slot0)
...
return {
	...
	OnBuySpecialWelfare = function (slot0, slot1, slot2)
		if isSuccess(slot1.result) then
			CommunityData:SetWealfareExchangeCnt(CommunityData:GetWelfareExchangeCnt() - slot2.num)
	...
	OnReceiveWelfare = function (slot0, slot1, slot2)
		if isSuccess(slot1.result) then
			...
			for slot7, slot8 in ipairs(slot1.give_items or {}) do
				table.insert(slot3, {
					id = slot8.id,
					num = slot8.num
				})
