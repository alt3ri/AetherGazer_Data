slot0 = class("BattleOsirisRaceTrialTemplate", BattleBaseStageTemplate)
...
function slot0.Ctor(slot0, slot1, slot2)
	...
	slot0.cfg_ = BattleRaceTrialStageCfg[ActivityRaceTrialCfg[slot1].stage_id]
...
function slot0.GetStageAffix(slot0)
	...
	for slot9, slot10 in ipairs(RaceTrialData:GetCustomList(slot0.activityID_)) do
...
function slot0.GetMaxRaceData(slot0)
	...
	for slot6, slot7 in ipairs(ActivityRaceTrialCfg.all) do
		if ActivityRaceTrialCfg[slot7].id == slot0.id_ then
			return slot1.race, 3
		end
	end
end
