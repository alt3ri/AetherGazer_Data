slot0 = singletonClass("RaceTrialData")
...
function slot0.InitData(slot0, slot1)
	...
	for slot7, slot8 in ipairs(ActivityData:GetActivityData(slot1.activity_id).subActivityIdList) do
...
function slot0.GetRaceTrialStartTime(slot0, slot1)
	return ActivityData:GetActivityData(slot1).startTime
end

function slot0.GetRaceTrialEndTime(slot0, slot1)
...
function slot0.RefreshBattleRedPoint(slot0, slot1)
	...
	manager.redPoint:setTip(string.format("%s_%s_%s", RedPointConst.ACTIVITY_RACE_TRIAL, RaceTrialTools.GetMainActivityID(slot1), slot1), ActivityData:GetActivityIsOpen(slot1) and slot0.battleData_[slot1].point <= 0 and 1 or 0)
end

function slot0.RefreshScoreRedPoint(slot0, slot1)
	slot2 = false
	slot3, slot4 = nil
	slot5 = RaceTrialTools.GetMainActivityID(slot1)

	for slot11, slot12 in ipairs(ActivityPointRewardCfg.get_id_list_by_activity_id[slot5]) do
		if ActivityPointRewardCfg[slot12].need <= slot0:GetTotalScore(slot5) and slot0.scoreRewardData_[slot12].receive_flag == false then
			slot2 = true
...
function slot0.RefreshFirstRedPoint(slot0, slot1)
	slot2 = false
	slot3 = RaceTrialTools.GetMainActivityID(slot1)
...
function slot0.SetBattleRedPointOn(slot0, slot1, slot2)
	manager.redPoint:setTip(string.format("%s_%s_%s", RedPointConst.ACTIVITY_RACE_TRIAL, RaceTrialTools.GetMainActivityID(slot2), slot2), 0)
end

function slot0.GetBattleResultGoal(slot0, slot1)
	slot4 = nil

	for slot8, slot9 in ipairs(slot0.battleData_[slot1].customList or {}) do
		slot3 = ActivityRaceTrialCfg[slot1].base_point + ActivityAffixPoolCfg[slot9].point
	end
