...
function slot0.RefreshUpSuit(slot0)
	...
	slot0.equipName_.text = GetI18NText(EquipSuitCfg[slot0.upSuitId_].name)

	SetSpriteWithoutAtlasAsync(slot0.equipIcon_, SpritePathCfg.EquipIcon_s.path .. slot0.upSuitId_)
end
...
function slot0.CreateLineItemList(slot0)
	...
	for slot5, slot6 in ipairs(slot0.stageList_) do
		if slot5 > 1 then
			slot0.lineList_[slot1] = slot0.lineList_[slot1] or slot0:GetLineClass(0).New(slot0.lineGo_, slot0.content_, slot0.pointGo_)

			slot0.lineList_[slot1]:Show(true)
			slot0.lineList_[slot1]:RefreshUI(slot0.missionItem_[slot5 - 1]:GetLocalPosition() + Vector3(0, 0, 0), slot0.missionItem_[slot5]:GetLocalPosition() + Vector3(0, 0, 0))
