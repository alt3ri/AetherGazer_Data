...
function slot0.AddListeners(slot0)
	slot0:AddBtnListener(slot0.button_, nil, function ()
		if table.keyof(EquipBreakThroughMaterialData:GetOpenModeList(), uv0.index_) == nil then
			ShowTips(string.format(GetTips("EQUIP_BREAK_THROUGH_MODE_UNLOCK"), GetI18NText(EquipBreakThroughMaterialDifficultyCfg[EquipBreakThroughMaterialDifficultyCfg[slot1].unlock_condition[2]].name)))
