...
function slot0.ClickItem(slot0)
	EquipBreakThroughMaterialAction.SendReceiveReward(EquipBreakThroughMaterialData:GetCanReceiveList(), function (slot0)
		uv0:RefreshUI()
	end)
end
...
function slot0.AddTimer(slot0)
	...
	slot0.timeText_.text = string.format(GetTips("REFRESH_LOST_TIME"), manager.time:GetLostTimeStr(EquipBreakThroughMaterialData:GetRefreshTimestamp()))
	slot0.timer_ = Timer.New(function ()
		uv0.timeText_.text = string.format(GetTips("REFRESH_LOST_TIME"), manager.time:GetLostTimeStr(uv1))
	end, 1, -1)
