		...
		if not manager.ChessManager.current:IsDetector() then
			if uv0.bugFixGo_ then
				...
				for slot6, slot7 in ipairs(uv1) do
					if math.abs(uv0.bugFixGo_.transform.localEulerAngles.y - slot7) < math.abs(999) then
						slot1 = slot8
						slot2 = slot7
					end
				end
			...
			slot0 = manager.ChessManager.current:LayDownOrUpShip(true, function ()
				uv0.shipBtnController_:SetSelectedState("2")
			end)
		else
			slot0 = manager.ChessManager.current:LayDownOrUpShip(false, function ()
...
function slot0.OnEnter(slot0)
	...
	if slot0.layoutTimer_ == nil then
		slot0.layoutTimer_ = Timer.New(function ()
			if ChapterCfg[uv0.params_.mapId] and slot0.cue_sheet ~= "" then
				manager.audio:PlayBGM(slot0.cue_sheet, slot0.cue_name, slot0.awb)
			end
...
function slot0.RefreshUI(slot0)
	slot0.topImg_.sprite = getSprite("Atlas/WarChess_BattleAtlas", WarchessLevelCfg[WarChessData:GetCurrentWarChessMapData().mapId].return_icon)
	slot0.btnPanelTrans_.offsetMax = Vector2.New(-ReduxView.VIEW_ADAPT_DISTANCE, 0)
end
...
function slot0.RefreshStepCountDown(slot0, slot1)
	if slot1 then
		slot0.stepcountdownText_.text = WarChessData:GetStepCountDownNum()
	end
...
function slot0.OnShipDie(slot0)
	...
	slot0.summerbalanceText_.text = GetTips("SUMMER_BALANCE_FAIL")
end
...
function slot0.OnChessSuccess(slot0)
	...
	slot0.summerbalanceText_.text = GetTips("SUMMER_BALANCE_WIN")
end
