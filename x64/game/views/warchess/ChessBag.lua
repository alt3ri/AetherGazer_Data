...
function slot0.InitUI(slot0)
	...
	slot0.scrollHelperKey_ = LuaList.New(handler(slot0, slot0.indexItemKey), slot0.itempanel_keyGo_, ChessBagKeyItem)
		...
		for slot6, slot7 in pairs(WarChessData:GetItemData()) do
			if WarchessObjectCfg[slot6].type ~= ChessConst.OBJECT_TYPE.ENERGY then
				slot0.itemList_[#slot0.itemList_ + 1] = {
					id = slot6,
					number = slot7,
					iconPath = "TextureConfig/WarChess_Battle/" .. slot8.icon
				}
			end
		end
	elseif slot1 == 2 then
