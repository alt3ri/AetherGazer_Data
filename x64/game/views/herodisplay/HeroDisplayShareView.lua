...
function slot1.OnSnape(slot0, slot1)
	slot3, slot4 = SettingTools.GetSettingScreenSize(tonumber(SettingData:GetSettingData().pic.resolution))

	if not SDKTools.IsSDK() then
		slot4 = _G.SCREEN_HEIGHT
		slot3 = _G.SCREEN_WIDTH
	end

	if slot1 then
		slot0.screenSnap_ = ScreenSnap.New(slot3, slot4)
	else
		slot0.screenSnap_ = ScreenSnap.New(slot4, slot3)
	end

	slot0.screenSnap_:Take()

	slot5 = slot0.screenSnap_:GetSprite()
	slot0.imageShot_.sprite = slot5
	slot0.imageBg_.sprite = slot5
...
function slot1.SendToSDK(slot0, slot1)
	if slot0.heroId_ then
		...
		if slot0.showWeapon_ and HeroDisplayData:GetHeroPoseList(slot0.heroId_)[slot0.posIndex_].weaponState ~= HeroDisplayConst.WEAPON_STATE.HIDE_WEAPON then
			slot3 = 1
		end
...
function slot1.ExitViewCallBack(slot0, slot1)
	if slot1 then
		slot0.exitViewFunc = slot1
	end
end
