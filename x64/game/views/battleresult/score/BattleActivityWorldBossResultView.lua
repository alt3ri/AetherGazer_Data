...
function slot0.RefreshMyExpS(slot0)
	slot2 = slot0.params_.battleResult.oldPlayerEXPInfo
	slot3 = slot2.userLevel
	slot4 = slot2.remain_exp

	if math.min(slot0.stageData:GetAddUserExp() * slot0.stageData:GetMultiple(), LvTools.GetMaxTotalExp("user_level_exp") - slot0:GetPlayerInfo().total_exp) < 0 then
		slot7 = 0
	end

	slot8, slot9 = LvTools.CheckExp(slot3, slot4 + slot7, "user_level_exp")

	if slot3 > 30 and slot8 - slot3 > 1 then
		error(string.format("经验计算有问题，玩家原来经验%s，添加了%s经验，lv: %s->%s", tostring(slot4), tostring(slot7), tostring(slot3), tostring(slot8)))
	end

	slot0.newPlayerExp_ = slot7
	slot0.addExp_.text = string.format("+%d", slot7)
	slot11 = math.floor(slot9 / GameLevelSetting[slot8].user_level_exp * 1000) / 1000

	if math.floor(slot4 / GameLevelSetting[slot3].user_level_exp * 1000) / 1000 > 0.99 then
		slot10 = 0.99
	end
	...
	slot12 = -1
	slot0.tween_ = LeanTween.value(slot0.progress_.gameObject, slot3 + slot10, slot8 + slot11, 2):setEase(LeanTweenType.easeOutCubic):setOnUpdate(LuaHelper.FloatAction(function (slot0)
