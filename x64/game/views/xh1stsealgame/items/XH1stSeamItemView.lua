...
function slot0.UpdateView(slot0)
	slot0.titleLabel_.text = GetI18NText(slot0.assignmentCfg_.desc)

	if slot0.assignmentCfg_.need < TaskData2:GetTask(slot0.assignmentId_).progress then
		slot1 = slot0.assignmentCfg_.need
	end

	slot0.progressBar_.value = math.min(1, slot1 / slot0.assignmentCfg_.need)
	...
	if TaskData2:GetTaskComplete(slot0.assignmentId_) then
