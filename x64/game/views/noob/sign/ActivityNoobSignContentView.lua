...
function slot0.SetData(slot0, slot1)
	...
	if slot0.lockController_ ~= nil then
		slot0.lockController_:SetSelectedIndex(math.floor(ActivityNoobData:GetSignDay() / 7) + 1 < slot0.pageIndex_ and 1 or 0)
	end

	slot0.rewardList_ = {}
	slot5 = (slot1 - 1) * 7 + 1

	for slot10 = slot5, slot5 + 7 - 1 do
		slot0.rewardList_[#slot0.rewardList_ + 1] = ActivityNewbieTools.GetSignCfg()[slot10]
	end
