slot0 = class("HeroTrialActivityItem", ReduxView)
...
function slot0.AddListeners(slot0)
	slot0:AddBtnListener(slot0.button_, nil, function ()
		HeroTrialData:SetSelectActivityHeroID(uv0.activityID_, uv0.acitviteTrialID_)
...
function slot0.RefreshUI(slot0)
	slot1 = HeroTrialTools.GetHeroStandardID(slot0.acitviteTrialID_)
	slot2 = HeroStandardSystemCfg[slot1].hero_id
	slot0.textName1_.text = string.format("%s·%s", GetI18NText(HeroCfg[slot2].name), GetI18NText(HeroCfg[slot2].suffix))
	slot0.textName2_.text = string.format("%s·%s", GetI18NText(HeroCfg[slot2].name), GetI18NText(HeroCfg[slot2].suffix))
	slot3 = SkinCfg[HeroStandardSystemCfg[slot1].skin_id].picture_id
