...
function slot0.UpdateHeroView(slot0)
	...
	if slot0.isAdvanceTest_ then
		slot0.fightPowerText_.text = AdvanceTestData:GetBattlePower(AdvanceTestData:GetAdvanceHeroData(slot0.selectID_))
	else
		slot0.fightPowerText_.text = getBattlePower(HeroData:GetHeroData(slot0.selectID_))
	end

	slot0.heroAvatarView_:SetSkinId(slot1.using_skin == 0 and slot1.id or slot1.using_skin)

	slot2 = HeroCfg[slot1.id]
	slot0.nameText_.text = GetI18NText(slot2.name)
	slot0.subNameText_.text = GetI18NText(slot2.suffix)
end
...
function slot0.GetHeroList(slot0)
	...
	return (not slot0.isAdvanceTest_ or HeroTools.Sort(AdvanceTestData:GetHeroList())) and HeroTools.Sort(HeroData:GetHeroList())
end
...
function slot0.RefreshCombo(slot0)
	...
	if slot0.selectID_ == slot0.heroTeam_[slot0.pos_] and slot0.heroTeam_[slot0.pos_] ~= 0 then
		deepClone(slot0.heroTeam_)[slot0.pos_] = 0
	else
