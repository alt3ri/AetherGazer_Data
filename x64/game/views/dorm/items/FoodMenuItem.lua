...
function slot0.RefreshItem(slot0, slot1, slot2, slot3)
	...
	if slot2 == DormEnum.MenuType.foodMenu then
		if not CanteenTools:CheckFoodUnLock(slot1) then
			...
			slot0.locktextText_.text = BackHomeTools:GetBackHomeConditionDes(BackHomeCanteenFoodCfg[slot1].unlock)
		else
			slot0.statusController:SetSelectedState("off")

			slot0.nameText_.text = GetI18NText(BackHomeCanteenFoodCfg[slot1].name)
	...
	elseif slot2 == DormEnum.MenuType.bag then
		...
		slot0.numText_.text = NumberTools.RetractNumberForWindBar(CanteenFoodData:GetCateenIngredient()[slot1])
