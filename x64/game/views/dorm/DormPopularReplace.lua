...
function slot0.AddUIListener(slot0)
	slot0:AddBtnListener(slot0.surebtnBtn_, nil, function ()
		if DormData:GetCurPopularEvent().id ~= 0 then
			ShowMessageBox({
				content = string.format(GetTips("DORM_POPULAR_EVENT_OPEN"), ItemTools.getItemName(slot0), ItemTools.getItemName(slot0)),
				OkCallback = function ()
...
function slot0.RefreshContent(slot0)
	slot0.descText_.text = ItemTools.getItemDesc(slot0.itemList_[slot0.selectItem_].id)
end
