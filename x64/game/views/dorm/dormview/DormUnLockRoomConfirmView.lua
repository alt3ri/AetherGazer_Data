...
function slot0.RefreshHeroList(slot0)
	...
	for slot5, slot6 in ipairs(DormHeroTools:GetCanUseHeroInDorm()) do
		if not DormRoomTools:GetDormIDViaArchive(slot6) and GameSetting.dorm_hero_set_level.value[1] <= DormData:GetHeroInfo(slot6):GetHeroFavorability() then
...
function slot0.indexItem(slot0, slot1, slot2)
	...
	slot2:PlaceHeroCallBack(function (slot0)
		ShowMessageBox({
			...
			OkCallback = function ()
				if DormData:GetHeroInfo(uv0):GetCanUseHeroList()[1] then
