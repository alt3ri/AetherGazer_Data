...
function slot0.refreshItem(slot0, slot1, slot2)
	...
	slot0.cursignfoodText_.text = #CanteenFoodData:GetChooseFoodList()
...
function slot0.UpdateItemData(slot0)
	...
	for slot4, slot5 in pairs(BackHomeCanteenFoodCfg.all) do
		if slot0.selectIndex_ == BackHomeCanteenFoodCfg[slot5].cook_type then
	...
	slot4 = CanteenTools
	slot4 = slot4.GetCookCfgName
	slot0.cooktitleText_.text = GetI18NText(slot4(slot4, slot0.selectIndex_))
...
function slot0.OnEnter(slot0)
	...
	if manager.guide:IsPlaying() then
		slot0.selectIndex_ = DormEnum.FurnitureTypeNum.Pan
	end
...
function slot0.RefreshFoodList(slot0)
	...
	slot0.chooseItemList_ = CanteenFoodData:GetChooseFoodList()
