...
function slot0.indexItem(slot0, slot1, slot2)
	slot2:SetData(slot1, AbyssCfg[slot0.idList_[slot1]].level)
end
...
function slot0.UpdateView(slot0)
	...
	slot0.passLabel_.text = GetI18NText(AbyssData:GetMaxLayer())
...
function slot0.UpdateList(slot0)
	...
	table.insertto(slot0.idList_, AbyssData:GetCurrentIdList(), 1)
...
function slot0.GetBonusStatus(slot0, slot1)
	if AbyssData:IsLayerBonusGet(AbyssCfg[slot1].level) then
		return 3
	elseif slot3 <= AbyssData:GetMaxLayer() then
		return 1
	else
		return 2
	end
