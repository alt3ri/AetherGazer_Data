...
function slot0.indexItem(slot0, slot1, slot2)
	slot3 = AbyssData:GetStageData(slot0.cfg_.level, slot0.index_)

	slot2:SetData(slot1, slot0.lockHeroIDList_[slot1])
end
...
function slot0.AddEventListeners(slot0)
	slot0:RegistEventListener(ABYSS_STAGE_RESET_TIME_UPDATE, function (slot0, slot1)
		...
		if slot0 == uv0.cfg_.level and slot1 == uv0.stageId_ then
			uv0:UpdateView()
		end
	end)
end
...
function slot0.UpdateView(slot0)
	...
	if AbyssData:IsLayerUnlock(slot0.cfg_.level) then
		slot0.unlockController_:SetSelectedState("true")

		slot0.scoreLabel_.text = AbyssData:GetStageScore(slot0.cfg_.level, slot0.index_)

		if AbyssData:GetStageData(slot0.cfg_.level, slot0.index_).is_completed then
			...
			slot0.lockHeroIDList_ = AbyssTools.GetStageLockHeroList(slot0.cfg_.level, slot0.index_)

			slot0.uiList_:StartScroll(#slot0.lockHeroIDList_)
		else
			slot0.challengeStatusController_:SetSelectedState("no")
		end

		if slot0.stageType_ == 3 then
			slot2 = AbyssData:GetStageData(slot0.cfg_.level, slot0.index_)
			...
			slot0.abandonIdList_ = AbyssTools.GetStageBanHeroList(slot0.cfg_.level, slot0.index_)
