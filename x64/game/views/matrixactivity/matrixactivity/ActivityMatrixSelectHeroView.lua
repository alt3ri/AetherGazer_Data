	...
	slot0:AddBtnListener(slot0.m_btn, nil, function ()
		...
		if uv0.heros[uv0.selectIndex].hero_type == 1 then
			...
			for slot14 = #EquipTools.CountActiveSuit(slot7.equip), 1, -1 do
				if slot10[slot14].num < EquipTools.GetSuitNeedNum(slot10[slot14].id, HeroTools.GetIsSuitNumCut(slot7)) then
			...
			slot4 = #TempHeroData:GetTempHeroDataByTempID(slot5.id).using_astrolabe > 0
			slot8 = HeroTools.GetIsSuitNumCut(slot6)
			slot9 = {}

			for slot13, slot14 in pairs(slot6.equip_list) do
		...
		if slot6 then
			ShowMessageBox({
				...
				content = slot6,
				OkCallback = handler(uv0, function ()
					if uv0.type == 1 then
...
function slot0.OnEnter(slot0)
	...
	for slot8, slot9 in pairs(HeroData:GetHeroList()) do
		...
		if slot9.unlock == 1 and slot10 and table.indexof(slot1.race_list, slot10.race) then
			table.insert(slot0.heros, {
				hero_type = 1,
				id = slot8
			})
