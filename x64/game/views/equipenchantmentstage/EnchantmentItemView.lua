...
function slot0.RefreshUI(slot0, slot1, slot2)
	...
	slot0.bossImg_.sprite = getSpriteWithoutAtlas(string.format("TextureConfig/ChapterPaint/EquipEnchantment/%s", slot0.stageImage_))

	if slot0.heroAffix_ and slot0.heroAffix_ ~= "" and slot0.heroAffix_[1] then
		slot0.heroaffixText_.text = GetI18NText(getAffixName(slot0.heroAffix_[1]))
	...
	if slot0.enemyAffix_ and slot0.enemyAffix_ ~= "" and slot0.enemyAffix_[1] then
		slot0.enemyaffixText_.text = GetI18NText(getAffixName(slot0.enemyAffix_[1]))
	...
	for slot6, slot7 in pairs(slot0.dropList_) do
		if not slot0.itemS[slot6] then
			...
			slot0.itemS[slot6]:RefreshData(rewardToItemTemplate(slot7))
			slot0.itemS[slot6]:HideNum()
		else
			slot0.itemS[slot6]:RefreshData(slot8)
			slot0.itemS[slot6]:HideNum()
		end
	end
end
