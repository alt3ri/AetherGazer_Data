	...
	slot0:AddBtnListener(slot0.receiveBtn_, nil, function ()
		...
		SoloChallengeAction.RequireReceive(uv0.activityID_, uv0.difficultyIndex_, function (slot0)
			if isSuccess(slot0.result) then
				getReward(formatRewardCfgList(ActivitySoloChallengeCfg[uv0.activityID_].reward_item_list[uv0.difficultyIndex_][2]))
...
function slot0.RefreshUI(slot0)
	slot1 = ""
	slot0.descText_.text = GetI18NText((slot0.difficultyIndex_ ~= 1 or GetTips("CLEAR_NORMAL_DIFFICULTY")) and (slot0.difficultyIndex_ ~= 2 or GetTips("CLEAR_DIFFICULTY")) and GetTips("CLEAR_EXTREME_DIFFICULTY"))
