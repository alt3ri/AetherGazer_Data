...
function slot0.IndexItem(slot0, slot1, slot2)
	...
	if (slot0.ownAffixID_ == slot3 or slot0.ownAffixID_ == 0 and slot2.stageID_ == -1) and not slot0.selected_ then
		slot4 = true
...
function slot0.AddUIListener(slot0)
	...
	slot0:AddBtnListener(slot0.dischargeBtn_, nil, function ()
		...
		SoloChallengeTools.DischargeAffix(uv0.activityID_, uv0.selectedAffixID_)
	end)
...
function slot0.RefreshIcon(slot0)
	if slot0.selectedAffixID_ ~= 0 then
...
function slot0.EquipAffix(slot0, slot1, slot2, slot3, slot4, slot5)
	if slot5 ~= -1 then
		SoloChallengeTools.DischargeAffix(slot1, slot4)
		SoloChallengeTools.EquipAffix(slot1, slot2, slot3, slot4)
	else
		SoloChallengeTools.EquipAffix(slot1, slot2, slot3, slot4)
	end
end

function slot0.OnSoloChallengeAffixRefresh(slot0, slot1)
	slot0:RefreshAffixCountText()
	slot0:RefreshAffixState(slot1)
	slot0:RefreshBtnState()
	slot0:SelectAffix(slot1)
end
...
function slot0.RefreshBtnState(slot0)
	...
	if slot0.selectedAffixID_ ~= 0 and (slot0.ownAffixID_ == slot0.selectedAffixID_ or SoloChallengeData:GetAffixState(slot0.activityID_, slot0.selectedAffixID_).stageID == slot0.stageID_) then
		slot0.buttonController_:SetSelectedState("discharge")
	elseif slot0.ownAffixID_ == 0 or slot1 then
