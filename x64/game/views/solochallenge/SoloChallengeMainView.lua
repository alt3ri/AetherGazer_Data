...
function slot0.RefreshSwitchItem(slot0)
	...
	for slot6, slot7 in ipairs(slot0.subActivityList_) do
		if slot0.switchItemList_[slot6] == nil then
			slot0.switchItemList_[slot6] = slot0:GetSwitchItemClass().New(slot0.switchItem_, slot0.switchPanel_, slot7, slot0.activityID_)
...
function slot0.RefreshRoleImage(slot0)
	slot0.roleImage_.sprite = getSpriteWithoutAtlas("TextureConfig/SummerUI/" .. ActivitySoloChallengeCfg[slot0.selectActivityID_].background)
...
function slot0.RefreshDesc(slot0)
	slot0.nameText_.text = GetI18NText(ActivitySoloChallengeCfg[slot0.selectActivityID_].name)
	slot0.descText_.text = GetI18NText(ActivitySoloChallengeCfg[slot0.selectActivityID_].desc)
end
