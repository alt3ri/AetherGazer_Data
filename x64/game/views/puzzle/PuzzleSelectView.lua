...
function slot0.RefreshUI(slot0)
	if slot0.retry_ == false then
		slot0:RefreshReward(formatRewardCfgList(ActivityPuzzleCfg[slot0.activityID_].reward_item_list))
...
function slot0.AddUIListener(slot0)
	...
	slot0:AddBtnListener(slot0.receiveBtn_, nil, function ()
		...
		PuzzleAction.Operate(uv0.activityID_, uv0.operationType_.receive, function (slot0, slot1)
			getReward(slot1.reward_list)
			PuzzleData:SetReceivedActivityList(uv0.activityID_)
			uv0:RefreshReward(slot1.reward_list)
		end)
	end)
	...
	slot0:AddBtnListener(slot0.playStoryBtn_, nil, function ()
		...
		if StoryCfg[ActivityPuzzleCfg[uv0.activityID_].story_id].trigger[4] == 1 then
			...
			manager.video:Play(slot1.videoPath, function ()
