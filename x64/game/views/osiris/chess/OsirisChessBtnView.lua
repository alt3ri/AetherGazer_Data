slot0 = class("OsirisChessBtnView", ReduxView)
...
function slot0.OnEnter(slot0)
	...
	manager.redPoint:bindUIandKey(slot0.transform_, string.format("%s_%s_%s", RedPointConst.WAR_CHESS, ActivityConst.OSIRIS_CHESS, slot0.chapterID_))
	slot0:AddTimer()
end

function slot0.OnExit(slot0)
	manager.redPoint:unbindUIandKey(slot0.transform_, string.format("%s_%s_%s", RedPointConst.WAR_CHESS, ActivityConst.OSIRIS_CHESS, slot0.chapterID_))
	slot0:StopTimer()
end
...
function slot0.AddListeners(slot0)
	slot0:AddBtnListener(slot0.button_, nil, function ()
		...
		if uv0:IsPrestageLock() then
			...
			ShowTips(string.format(GetTips("UNLOCK_CONDITION_ACTIVITY_TIPS"), GetI18NText(ActivityCfg[WarchessLevelCfg[slot2].type].remark), WarchessLevelCfg[slot2].name_level))
...
function slot0.IsLock(slot0)
	if WarchessLevelCfg[slot0.chapterID_].type == ActivityConst.OSIRIS_CHESS and WarchessLevelCfg[slot1].unlock_condition ~= "" then
