...
slot0 = class("OsirisStoryStageInfoView", SectionInfoBaseView)
...
function slot0.OnClickBtn(slot0)
	...
	if BattleConst.BATTLE_TAG.STORY == BattleStageTools.GetStageCfg(slot0.stageType_, slot0.stageID_).tag then
		BattleController.GetInstance():LaunchStoryBattle(slot0.stageType_, slot0.stageID_, ActivityStoryChapterCfg[slot0.params_.chapterID].activity_id)
	else
