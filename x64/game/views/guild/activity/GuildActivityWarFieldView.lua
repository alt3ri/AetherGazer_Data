...
function slot0.UpdateLevelView(slot0)
	slot0.mapNameLabel_.text = GetI18NText(ActivityClubMapCfg[ActivityClubMapCfg.all[slot0.curMapIndex_]].map_name)
	...
	slot0.profitLabel_.text = string.format("%d(+%d/%s)", ItemTools.getItemNum(MaterialConst.GUILD_ACTIVITY_COIN), GuildActivityData:GetCoinGain(), GetTips("HOUR"))
...
function slot0.ChatNewMessage(slot0)
	if #ChatChannelData:GetCacheContent(ChatConst.CHAT_CHANNEL_GUILD_ACTIVITY) > 0 then
		...
		if slot2.contentType == ChatConst.CHAT_CONTENT_TYPE.STICKER then
			slot3 = string.format("[%s]", GetI18NText(ChatStickerCfg[tonumber(slot2.content)].name))
		end
...
function slot0.RefreshBossSprite(slot0)
	slot4 = {}

	ComponentBinder.GetInstance():BindCfgUI(slot4, GuildActivityLuaBridge.GetManager().gameObject.transform:GetChild(0))

	if slot4.hpSprite_ then
		slot4.hpSprite_.sprite = getSprite("Atlas/GuildActivitiesUI", string.format("%s@%s", ({
		...
		}).hpSprite, SettingData:GetCurrentLanguage()))
	end

	if slot4.fightingSprite_ then
		slot4.fightingSprite_.sprite = getSprite("Atlas/GuildActivitiesUI", string.format("%s@%s", slot3.fightingSprite, slot5))
	end

	if slot4.defeatSprite_ then
		slot4.defeatSprite_.sprite = getSprite("Atlas/GuildActivitiesUI", string.format("%s@%s", slot3.defeatSprite, slot5))
	end

	if slot4.nextLevelSprite_ then
		slot4.nextLevelSprite_.sprite = getSprite("Atlas/GuildActivitiesUI", string.format("%s@%s", slot3.nextLevel, slot5))
	end
end
