slot0 = class("GuildActivitySPNodeResultView", ReduxView)
...
function slot0.RefreshGuildActivityUI(slot0)
	slot2 = ActivityClubSPCfg[slot0.params_.nodeId]
	...
	if slot0.params_.isSuccess then
		...
		if slot2.type == 2 then
			slot5 = GuildActivitySPData:GetSelfNodeData(slot1).elite_health_rate == 0 and 100 or slot4.elite_health_rate
			slot6 = 0

			if LuaExchangeHelper.GetBattleStatisticsData().dataForLua.recordDatas then
				if slot7:TryGetValue(27, nil) then
					slot0.progressLabel_.text = slot5 - math.ceil(slot7[27] * slot5 / 100) .. "%"
				else
			...
			if 100 - math.ceil(slot7[27] * slot5 / 100) > 99 then
				slot8 = 99
			end

			slot0.totalProgressLabel_.text = slot8 .. "%"
		end
	end
end
