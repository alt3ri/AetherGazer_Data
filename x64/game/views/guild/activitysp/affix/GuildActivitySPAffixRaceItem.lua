slot0 = class("GuildActivitySPAffixRaceItem", ReduxView)
...
function slot0.BindRedPoint(slot0, slot1)
	if slot0.raceID_ ~= nil then
		manager.redPoint:bindUIandKey(slot0.transform_, string.format("%s_%d_%d", RedPointConst.GUILD_ACTIVITY_SP_AFFIX, slot1, slot0.raceID_))
	end
end

function slot0.UnBindRedPoint(slot0, slot1)
	if slot0.raceID_ ~= nil then
		manager.redPoint:unbindUIandKey(slot0.transform_, string.format("%s_%d_%d", RedPointConst.GUILD_ACTIVITY_SP_AFFIX, slot1, slot0.raceID_))
	end
end
