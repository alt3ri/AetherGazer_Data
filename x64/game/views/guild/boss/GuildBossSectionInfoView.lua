...
function slot0.OnEnter(slot0)
	...
	if slot0.guildStateController_ then
		slot0.guildStateController_:SetSelectedState("guildBoss")
	end

	if slot0.hideFatigueController_ then
...
function slot0.RefreshStageInfo(slot0)
	...
	if slot0.oldCfgID_ ~= slot0.cfg_.id then
		slot0.sectionName_.text = GetI18NText(ClubBossPreposeCfg[slot0.params_.index].name)
		...
		slot0.bossBuffDescLabel_.text = GetI18NText(getAffixDesc(slot2))
		slot0.bossBuffNameLabel_.text = GetI18NText(getAffixName(slot2))
