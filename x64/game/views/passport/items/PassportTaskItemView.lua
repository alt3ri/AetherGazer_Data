...
function slot0.UpdateTimer(slot0)
	...
	if (slot0.taskInfo_.expired_timestamp or 0) > 0 then
		...
		slot0.lastTimeLabel_.text = manager.time:GetLostTimeStr(slot1)
	else
...
function slot0.indexItem(slot0, slot1, slot2)
	CommonTools.SetCommonData(slot2, {
		...
		grayFlag = slot0.taskInfo_.complete_flag == 1,
	...
	slot0:AddBtnListener(slot0.getBtn_, nil, function ()
		...
		if slot0 and (uv0.taskCfg_.type == 7 or uv0.taskCfg_.type == 8) and GameSetting.battlepass_exp_limit_weekly.value[1] <= PassportData:GetExpWeekly() then
			ShowMessageBox({
				...
				OkCallback = function ()
					TaskAction:SubmitTask(uv0.taskCfg_.id)
				end,
...
function slot0.SetData(slot0, slot1)
	...
	slot0.taskInfo_ = TaskData2:GetTask(slot1)
...
function slot0.UpdateView(slot0)
	...
	slot0.typeLabel_.text = GetI18NText(slot0.taskCfg_.name)
	slot0.descLabel_.text = GetI18NText(slot0.taskCfg_.desc)

	if slot0.taskCfg_.need < TaskData2:GetTaskProgress(slot0.taskCfg_.id) then
		slot1 = slot2
	end

	slot0.progressLabel_.text = string.format("%d/%d", slot1, slot2)
	slot0.progressSlider_.value = slot1 / slot2
