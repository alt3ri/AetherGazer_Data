...
function slot0.OnEnter(slot0)
	...
	if slot0.params_.sectionType == BattleConst.STAGE_TYPE_NEW.STAGE_TYPE_CHESS or slot0.params_.sectionType == BattleConst.STAGE_TYPE_NEW.SAIL_GAME or slot0.params_.sectionType == BattleConst.STAGE_TYPE_NEW.STAGE_TYPE_NEWWARCHESS then
	...
	if not slot0.params_.reserveParams then
		slot0.params_.reserveParams = ReserveTools.GetReserveParams(slot0.stageType_, slot0.stageID_, slot0.params_.activityID)
	elseif not slot0.params_.reserveParams.customData.activityID then
		slot0.params_.reserveParams.customData.activityID = slot0.params_.activityID
	end
...
function slot0.InitUI(slot0)
	...
	for slot4 = 1, 3 do
		...
		slot0.hpGp_[slot4] = slot0[string.format("hp%sGo_", slot4)]
...
function slot0.CreateDragListeners(slot0, slot1)
	...
	slot0.drag_[slot1]:AddListenerType1(UnityEngine.EventSystems.EventTriggerType.Drag, LuaHelper.EventTriggerAction1(function (slot0, slot1)
		...
		if uv0.heroModel_[uv1] then
			uv0.heroModel_[uv1].transform.localPosition = uv2[1].position + uv0:GetModelOffesetPosition(slot0.transform.localPosition)
...
function slot0.TryModifyHeroPos(slot0)
	for slot6 = 1, 3 do
		if math.abs(uv0[slot6].position.x - slot0.heroModel_[slot0.selectIndex_].transform.localPosition.x) <= 0.6 and slot6 ~= slot0.cacheSelectIndex_ then
...
function slot0.GotoReservesUI(slot0)
	JumpTools.OpenPageByJump("/reservesUI", {
		...
		chipInfo = {
			enabledID = slot0.chipView_:GetCurMimirID(),
...
function slot0.StartBattle(slot0)
	slot1 = nil
	slot1 = (not slot0.roomProxy_ or slot0.roomProxy_:GetTemplate()) and BattleStageFactory.Produce(slot0.stageType_, slot0.stageID_, slot0.params_.activityID, slot0.reserveParams_)
...
function slot0.LoadHeroModels(slot0)
	for slot4, slot5 in ipairs(slot0.cacheHeroTeam_) do
		if slot5 ~= 0 and slot5 then
			...
			slot0.loadAsyncIndex_[slot4] = manager.resourcePool:AsyncLoad("Char/" .. slot0:GetSkinCfg(slot4, slot5).ui_modelId, ASSET_TYPE.TPOSE, function (slot0)
...
function slot0.RefreshAddBtn(slot0)
	...
	for slot4, slot5 in ipairs(slot0.cacheHeroTeam_) do
		...
		if slot5 and slot5 == 0 then
			...
			slot0.heroPowerIcon_[slot4].sprite = getSprite("Atlas/CampItemAtlas", RaceEffectCfg[HeroCfg[slot5].race].icon)
...
function slot0.RefreshHp(slot0)
	if slot0.params_.sectionType == BattleConst.STAGE_TYPE_NEW.STAGE_TYPE_NEWWARCHESS then
		for slot4, slot5 in pairs(slot0.cacheHeroTeam_) do
			if slot5 and slot5 == 0 then
				SetActive(slot0.hpGp_[slot4], false)
			elseif not NewWarChessData:GetHeroHp(slot5) then
				print("英雄血量不存在")
			else
				SetActive(slot0.hpGp_[slot4], true)

				slot0.hpImg_[slot4].fillAmount = slot6 / NewChessConst.HERO_HP_RATE
				slot0.hpText_[slot4].text = math.ceil(slot6 / NewChessConst.HERO_HP_RATE * 100) .. "%"
			end
		end
	else
		for slot4 = 1, 3 do
...
function slot0.RefreshRace(slot0)
	...
	if slot0.params_.forceRace then
		...
		slot0.effectName_.text = GetI18NText(slot5.name)
		slot0.effectImage_.sprite = getSprite("Atlas/CampItemAtlas", slot5.icon)
		slot0.effectHeroText_.text = GetI18NText(slot5.desc)
		slot0.effectText_.text = GetI18NText(slot5.battle_desc)
		...
		slot0.effectName_.text = GetI18NText(slot4.name)
		slot0.effectHeroText_.text = GetI18NText(slot4.desc)
		slot0.effectImage_.sprite = getSprite("Atlas/CampItemAtlas", slot4.icon)
	end
end
...
function slot0.RefreshChip(slot0)
	...
	if slot1 then
		slot0.chipView_:SetData(slot0.stageType_, slot0.stageID_, slot0.params_.activityID, slot0.reserveParams_)
	end
end

function slot0.RefreshCombo(slot0)
	slot0.comboSkillView_:SetHeroList(slot0.stageType_, slot0.stageID_, slot0.params_.activityID, slot0.cacheHeroTeam_, slot0.heroTrialList_, slot0.params_.index, slot0.reserveParams_)
end
...
function slot0.RefreshRecommend(slot0)
	if not slot0.params_.recommend_type then
		SetActive(slot0.recommendTypeGo_, false)
	...
	SetActive(slot0.recommendTypeGo_, slot4)

	for slot4, slot5 in ipairs(GameDisplayCfg.hero_atack_type_tag.value) do
		if slot5[1] == slot0.params_.recommend_type then
			slot0.recommendTypeIcon_.sprite = getSpriteWithoutAtlas("TextureConfig/HeroIconUI/AttributeIcon/" .. slot5[2])
...
function slot0.GoHeroInfoUI(slot0, slot1)
	...
	slot0:Go(slot0:GetHeroTeamInfoRoute(), {
		isEnter = true,
		selectHeroPos = slot1,
...
function slot0.ChangeHeroTeam(slot0, slot1, slot2)
	if slot0.roomProxy_ then
		slot0.roomProxy_:SetTeamInfo(slot2)
	else
		slot3 = slot1
		slot4 = {
			[slot8] = slot0.heroTrialList_[table.keyof(slot0.cacheHeroTeam_, slot9)]
		}
		...
		if slot3[1] ~= slot0.cacheHeroTeam_[1] then
			slot0:PlayHeroTalk(slot3[1])
		end

		ReserveTools.SetTeam(slot0.reserveParams_, slot3, slot4, slot0.comboSkillView_:GetCurComboSkill(), slot0.chipView_:GetCurMimirID(), slot0.chipView_:GetCurChipList())
	end
end

function slot0.SaveTeamInfo(slot0)
	if ReserveTools.NeedCacheTeam(slot0.stageType_, slot0.stageID_) then
		ReserveTools.SetTeam(slot0.reserveParams_, slot0.cacheHeroTeam_, slot0.heroTrialList_, slot0.comboSkillView_:GetCurComboSkill(), slot0.chipView_:GetCurMimirID(), slot0.chipView_:GetCurChipList())
	end
end
...
function slot0.GetTotalPower(slot0)
	for slot5, slot6 in pairs(slot0.cacheHeroTeam_) do
		if slot6 ~= 0 then
			slot1 = 0 + slot0:GetHeroPower(slot5)
		end
	end
