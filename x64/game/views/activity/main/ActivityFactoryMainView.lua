...
function slot0.InitSubmoduleButton(slot0)
	slot0.submoduleBtnList_ = {
		...
		slot0:GetHeroTrialItemClass().New(slot0.heroTrialGo_, ActivityConst.FACTORY_HERO_TRIAL),
		slot0:GetSkinTrialItemClass().New(slot0.heroSkinGo_, ActivityConst.FACTORY_SKIN_TRIAL),
		ActivitySubmoduleItem.New(slot0.soloBtn7_, ActivityConst.FACTORY_SOLO_CHALLENGE)
	}
end

function slot0.GetHeroTrialItemClass(slot0)
...
function slot0.GetSkinTrialItemClass(slot0)
	if not SDKTools.GetIsOverSea() then
