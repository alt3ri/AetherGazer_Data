...
function slot0.SdkRecord(slot0)
	if not manager.time:IsToday(getData("activity_last_click", "activity_" .. slot0.activityID_) or 0) then
		OperationRecorder.RecordButtonTouch("activity_homepage_" .. slot0.activityID_)
		saveData("activity_last_click", "activity_" .. slot0.activityID_, manager.time:GetServerTime())
	end
end
...
function slot0.OnSelect(slot0, slot1)
	...
	if slot1 then
		ActivityRedPoint:CancelSubActivityNewTag(slot0.activityID_)
	end
...
function slot0.UpdateActivity(slot0, slot1)
	if slot0:GetTempActivityID() == slot1 then
		slot3 = ActivityData:GetActivityData(slot2)
		slot0.startTime_ = slot3.startTime
		slot0.stopTime_ = slot3.stopTime

		slot0:AddTimer()
	end
end

function slot0.GetTempActivityID(slot0)
	if ActivityShopCfg[slot0.activityID_] then
		return ShopListCfg[ActivityShopCfg[slot0.activityID_].shop_id].activity_id
	end
...
function slot0.AddTimer(slot0)
	...
	slot0.timer_ = Timer.New(function ()
		if uv0 and uv1.startTime_ < manager.time:GetServerTime() then
			uv1:RefreshLock()
...
function slot0.RefreshUI(slot0)
	...
	slot0.iconImage_.sprite = getSpriteWithoutAtlas("TextureConfig/VersionUI/ActivityiconUI/" .. slot1.icon)
end
