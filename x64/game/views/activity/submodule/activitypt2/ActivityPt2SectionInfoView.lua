...
function slot0.AddListeners(slot0)
	...
	slot0:AddBtnListener(slot0.battleBtn_, nil, function ()
		...
		elseif uv0.costId ~= 0 and ItemTools.getItemNum(uv0.costId) < uv0.cost * uv0.multiple_ then
			if uv0.costId == CurrencyConst.CURRENCY_TYPE_VITALITY then
				...
				ShowTips(string.format(GetTips("CURRENCY_NO_ENOUGH"), ItemTools.getItemName(uv0.costId)))
			end
		else
			uv0:OnClickBtn()
		end
...
function slot0.UpdateBar(slot0)
	slot1 = ActivityPt2Tools.GetChallengeCurrencyID(slot0.params_.mainActivityId)
...
function slot0.RefreshData(slot0)
	...
	for slot5 = #GameSetting.challenge_multiple_count_unlock_level.value, 1, -1 do
		if ItemTools.getItemNum(slot0.costId) >= slot0.cost * slot5 then
			slot1 = slot5
