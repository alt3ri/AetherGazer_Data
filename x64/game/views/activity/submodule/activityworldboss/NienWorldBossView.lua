...
function slot0.OnEnter(slot0)
	...
	if ActivityWorldBossCfg[slot0.activity_id].pre_story_id and not manager.story:IsStoryPlayed(slot2) then
		manager.story:StartStoryById(slot2, function (slot0)
		end)
	end
...
function slot0.RefreshUI(slot0)
	...
	slot9 = slot2 / 10
	slot0.m_bossName.text = GetMonsterName({
		slot1.boss_id[1]
	}) .. " " .. math.ceil(slot9) .. "%"

	for slot9 = 1, #slot1.health_reward_list do
		if not slot0.rewardItems[slot9] then
			slot0.rewardItems[slot9] = NiewWorldBossRewardItem.New(Object.Instantiate(slot0.m_rewardItem, slot0.m_rewardContent), slot0.m_rewardContent.rect.width)
		end

		slot0.rewardItems[slot9]:SetData(slot0.activity_id, slot9)
		slot0.rewardItems[slot9]:SetActive(true)
	end

	for slot9 = slot4 + 1, #slot0.rewardItems do
		slot0.rewardItems[slot9]:SetActive(false)
	end
	...
	if ActivityWorldBossData:GetAffix(slot0.activity_id) then
		...
		slot0.m_affixDes.text = GetI18NText(getAffixDesc(slot8))
		slot0.m_affixImg.sprite = getAffixSprite(slot8)
	else
