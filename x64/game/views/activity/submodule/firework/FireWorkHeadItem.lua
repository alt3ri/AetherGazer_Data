...
function slot0.SetData(slot0, slot1)
	slot0.id = slot1
	slot0.m_icon.sprite = getSpriteWithoutAtlas("TextureConfig/EmptyDream/letter_role_title/role_small/" .. ActivityFireWorkCfg[slot1].icon)
...
function slot0.RefrenTime(slot0)
	if ActivityData:GetActivityIsOpen(slot0.id) then
		...
		if ActivityData:GetActivityData(slot0.id) and manager.time:GetServerTime() < slot2.startTime then
