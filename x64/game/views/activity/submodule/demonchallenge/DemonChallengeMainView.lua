...
function slot0.LevelRenderer(slot0, slot1, slot2)
	...
	if slot0.iconS_[slot1] then
		slot0.iconS_[slot1].sprite = getSpriteWithoutAtlas("TextureConfig/VersionUI/" .. DemonChallengeCfg[DemonChallengeTools.GetChildId(slot1, 1)].icon)
	end
end
...
function slot0.RefreshTime(slot0)
	if ActivityData:GetActivityData(slot0.activityId_):GetRemainTime() >= 0 then
		slot0.remainTimeText_.text = string.format(GetTips("TIME_DISPLAY_3"), manager.time:GetLostTimeStr(slot1.stopTime))
	end
