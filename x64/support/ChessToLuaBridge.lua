...
function PreloadChessAsset()
	slot1 = nil
	slot5 = WarChessData:GetCurrentActivity()
...
function OnExitChessScene(slot0)
	...
	if slot0 then
		...
		for slot9, slot10 in pairs(slot8.GetItemData(slot10)) do
			table.insert({}, {
				slot9,
				slot10
			})
		...
		for slot10, slot11 in pairs(slot9.GetHeroList(slot11)) do
			table.insert({}, {
				slot10,
				slot11
			})
		...
		for slot11, slot12 in pairs(slot10.GetArtifactData(slot12)) do
			table.insert({}, slot11)
		end
