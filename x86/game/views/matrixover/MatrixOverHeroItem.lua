...
function slot0.Refresh(slot0, slot1)
	...
	slot7 = slot2.hero_id
	slot0.icon_.sprite = HeroTools.GetHeadSprite(slot0:GetHeroSkin(slot7))

	slot0.servantView_:Refresh(slot2:GetWeaponServant())

	for slot7, slot8 in ipairs(slot0.equipItemView_) do
		slot8:Refresh(slot2:GetEquipList()[slot7])
	end

	for slot8, slot9 in ipairs(slot0.astrolabeItemView_) do
		slot9:Refresh(slot2:GetAstrolabeList()[slot8], slot0.heroID_)
	end
end
