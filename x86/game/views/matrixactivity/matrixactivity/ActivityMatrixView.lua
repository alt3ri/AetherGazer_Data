...
function slot0.StartMatrix(slot0)
	...
	if slot3 ~= 0 and not ActivityMatrixData:GetIsClearance(slot3) then
		ShowTips(string.format(GetTips("ACTIVITY_MATRIX_PRECONDITIONS"), GetI18NText(ActivityMatrixCfg[slot3].name)))
...
function slot0.RefreshUI(slot0)
	...
	slot0.m_scoreLab.text = ActivityMatrixData:GetMatrixScore(slot1)
	...
	elseif MatrixConst.STATE_TYPE.NOTSTARTED ~= ActivityMatrixData:GetGameState(slot1) then
		...
		slot0.m_enterText_1.text = GetTips("ACTIVITY_MATRIX_PERFORM_TIPS")
		slot0.m_enterText_2.text = GetTips("ACTIVITY_MATRIX_PERFORM_TIPS")
		slot0.m_enterText_3.text = GetTips("ACTIVITY_MATRIX_PERFORM_TIPS")
	else
		slot0.particleController:SetSelectedIndex(1)

		slot0.m_enterText_1.text = GetTips("ACTIVITY_MATRIX_ENTER_TIPS")
		slot0.m_enterText_2.text = GetTips("ACTIVITY_MATRIX_ENTER_TIPS")
		slot0.m_enterText_3.text = GetTips("ACTIVITY_MATRIX_ENTER_TIPS")
	end

	slot0.m_titleText.text = GetI18NText(slot3.name)
	...
	manager.redPoint:SetRedPointIndependent(slot0.m_rightBtn.transform, slot10)
...
function slot0.RefrenTime(slot0)
	if ActivityData:GetActivityData(slot0.main_matrix_activity_id) and manager.time:GetServerTime() < slot2.stopTime then
	...
	if slot5 ~= 0 and not ActivityMatrixData:GetIsClearance(slot5) then
		...
		slot0.m_timeLab.text = string.format(GetTips("ACTIVITY_MATRIX_PRECONDITIONS"), GetI18NText(ActivityMatrixCfg[slot5].name))
