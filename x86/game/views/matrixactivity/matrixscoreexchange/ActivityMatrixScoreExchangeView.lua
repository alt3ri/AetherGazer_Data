...
function slot0.AddUIListener(slot0)
	slot0:AddBtnListener(slot0.oneKeyGetBtn_, nil, function ()
		...
		for slot5, slot6 in ipairs(uv0.rewards) do
			if ActivityPointRewardCfg[slot6].need <= ActivityMatrixData:GetMatrixScore(uv0.matrix_activity_id) and not ActivityMatrixData:GetMatrixPointRewardState(uv0.matrix_activity_id, slot6) then
...
function slot0.Refresh(slot0)
	slot0.myScoreText_.text = string.format("%d", ActivityMatrixData:GetMatrixScore(slot0.matrix_activity_id))
	slot2 = false
	...
	slot0.m_name.text = GetI18NText(ActivityMatrixCfg[slot0.matrix_activity_id].name)
end
