...
function slot0.RefreshUI(slot0, slot1, slot2)
	...
	if slot3 == 0 then
		...
		if slot2 == 1 then
			slot0.nameText_.text = GetI18NText(ChapterClientCfg[ChapterCfg[slot1.chapterID].clientID].name)
		...
		elseif slot2 == 2 then
			slot0.nameText_.text = GetI18NText(ActivityCfg[slot1.chapterID].remark)
			slot0.reveiveText_.text = slot3 .. "/" .. slot1.all
		...
		elseif slot2 == 3 then
			slot4 = slot0.info_.storyList[1]
			slot0.nameText_.text = GetI18NText(StoryCfg[slot4].name)
			slot0.reveiveText_.text = slot3 .. "/" .. slot1.all

			getSpriteWithoutAtlasAsync(SpritePathCfg.CollectPlotSmall.path .. CollectStoryCfg[slot4].picture, function (slot0)
				uv0.iconImg_.sprite = slot0
			end)
		end
	end
...
function slot0.AddUIListener(slot0)
	slot0:AddBtnListener(slot0.itemBtn_, nil, function ()
		...
		elseif uv0.selType_ == 1 then
			...
			ShowTips(string.format(GetTips("MISSION_PROGRESS_UNLOCK"), getChapterDifficulty(slot1), GetI18NText(slot4), GetI18NText(slot5)))
		end
	end)
end
