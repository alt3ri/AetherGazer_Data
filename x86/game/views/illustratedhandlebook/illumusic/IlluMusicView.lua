...
function slot0.OnAlbumClick(slot0, slot1)
	...
	slot0.detail_list:ScrollToIndex(slot0.pageIndex, true, slot6)
...
function slot0.GetMusicId(slot0)
	if GetHomeMusicID() ~= 0 and MusicRecordCfg[slot1] then
		return slot1
	else
		if slot0.player then
			if (slot0.player.cueName or "") ~= "" and (slot0.player.cueSheet or "") ~= "" then
				for slot8, slot9 in ipairs(MusicRecordCfg.all) do
					if MusicRecordCfg[slot9].cueName == slot2 and slot10.cuesheet == slot3 then
						return slot9
					end
				end
			end
...
function slot0.PlayMusic(slot0, slot1)
	...
	if slot0.player then
		...
		for slot7, slot8 in pairs(MusicData:GetAisacSet(slot0.music_id)) do
			AudioManager.Instance:SetAisacControlOfCategory("music", slot7, slot8)
		end
	end
...
function slot0.RefreshState(slot0)
	...
	slot0.m_musicName.text = GetI18NText(slot1.musicName)
	slot0.m_author.text = GetI18NText(slot1.authorName)

	for slot6, slot7 in pairs(slot0.detail_list:GetItemList()) do
		slot7:RefreshSongsState(slot0.music_id)
	end

	if slot0.music_id == GetHomeMusicID() then
