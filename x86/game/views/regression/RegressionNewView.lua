...
function slot1.OnEnter(slot0)
	...
	elseif slot0.params_.isBack == true then
		...
		if slot0.params_.lastItemIndex > 0 then
			slot0.tree:SelectItem(slot0.params_.lastIndex, slot2)
		else
...
function slot1.SetSelectPage(slot0, slot1, slot2)
	...
	slot3 = slot0.params_.isBack

	if slot0.params_.isBack == true then
		if slot0.curPage.SetIsBack then
	...
	slot0.curPage:UpdateBar()
	slot0:PlayAnim(slot3)
end

function slot1.PlayAnim(slot0, slot1)
	slot2 = false
	slot3 = -1
	...
	if slot0.params_.isEnter == true then
		if slot0.needExitAnim_[slot0.selectIndex] == true then
			slot2 = false
			slot3 = slot0.playAnimTypeConst_.EXIT
		elseif slot0.needEnterAnim_[slot0.selectIndex] == true then
			slot2 = true
			slot3 = slot0.playAnimTypeConst_.ENTER
			slot0.params_.isEnter = false
		end
	elseif slot0.needEnterAnim_[slot0.selectIndex] == true and slot0.lastPlayType_ ~= slot0.playAnimTypeConst_.ENTER then
		slot3 = slot0.playAnimTypeConst_.ENTER
		slot2 = true
	...
	SetActive(slot0.animPanelGo_, slot2)

	if slot2 == true and slot3 ~= -1 then
		slot0.lastPlayType_ = slot3

		slot0.animation_:Play(slot0.animConst_[slot3])
	end
end
