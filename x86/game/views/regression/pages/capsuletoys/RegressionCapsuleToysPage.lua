...
function slot0.OnCtor(slot0, slot1)
	slot3 = Object.Instantiate(Asset.Load("Widget/System/ReturnTwo/RT2stTwistedEggUI"), slot1)
...
function slot0.AddUIListener(slot0)
	slot0:AddBtnListener(slot0.drawOnceBtn_, nil, function ()
		...
		if slot0.value[2] <= ItemTools.getItemNum(slot0.value[1]) then
	...
	slot0:AddBtnListener(slot0.drawMultipleBtn_, nil, function ()
		...
		if ItemTools.getItemNum(slot0.value[1]) >= slot0.value[2] * 2 then
			manager.audio:PlayEffect("ui_system", "back_niudan")
			uv0.drawController_:SetSelectedState("true")
...
function slot0.OnCapsuleToysDraw(slot0, slot1, slot2)
	if isSuccess(slot1.result) then
		...
		for slot8, slot9 in ipairs(slot1.item_list) do
			...
			slot12 = nil
			slot12 = (slot10 ~= 0 or GameSetting.regression_gashapon_reward.value) and RegressionCapsuleToysCfg[slot10].reward
			slot4[#slot4 + 1] = {
				slot12[1],
				slot12[2] * slot11
			}
		...
		RegressionData:SetCapsuleToysStatus(slot3)
...
function slot0.RefreshMultiDrawBtn(slot0)
	...
	if ItemTools.getItemNum(slot1.value[1]) >= slot1.value[2] * 2 then
