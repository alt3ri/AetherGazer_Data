...
function slot0.OnEnter(slot0)
	...
	if getData("affixSelect", "scrollPos" .. slot0.params_.activityId) then
		...
		slot0.itemScrollRect_.verticalNormalizedPosition = 1
	end
end
...
function slot0.UpdateView(slot0)
	...
	slot0.bossIndexController_:SetSelectedState(tostring(slot4))
	...
	slot0.enemyAffixLabel_.text = GetI18NText(getAffixDesc(slot0.cfg_.boss_affix))
...
function slot0.UpdateGainScore(slot0)
	for slot5, slot6 in ipairs(slot0.selectedAffixList_) do
		slot1 = slot0.cfg_.base_point + ActivityAffixPoolCfg[slot6].point
	end
