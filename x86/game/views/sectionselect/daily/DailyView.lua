...
function slot0.RefreshUI(slot0)
	...
	if not slot0:IsOpenSectionInfo() and not slot0:IsOpenRoute("chapterSectionReward") and #getChapterClientCfgByChapterID(slot0.chapterID_).chapter_list > 1 then
...
function slot0.SwitchDailyChapter(slot0, slot1)
	...
	if slot0.params_.section == (BattleFieldData:GetCacheStage(slot1) or GetI18NText(ChapterCfg[slot1].section_id_list[1])) then
		return
	end
