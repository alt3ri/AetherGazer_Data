...
function slot0.OnEnter(slot0)
	slot0:RegistEventListener(DORM_LOGIC_TICK, handler(slot0, slot0.Update))
...
function slot0.InitBubbleManager(slot0)
	...
	for slot6, slot7 in Dorm.storage:ForeachData(CanteenItemManager.ItemEIdNamespace(DormEnum.ItemType.DinningTable), slot6) do
...
function slot0.FindAvailableCustomerInRange(slot0, slot1)
	...
	for slot7, slot8 in Dorm.storage:ForeachData(CanteenCharacterManager.EIdNamespace(slot2), slot7) do
		if CanteenAIFunction:GetEntityData(slot8).pay then
			for slot16, slot17 in pairs(slot9.peer) do
				...
				slot10 = false or slot0:InRange(slot17)
				slot11 = true and slot18.sit
				slot12 = false or slot18.eat
			end
