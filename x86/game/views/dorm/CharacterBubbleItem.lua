...
function slot0.RegisterEvent(slot0)
	manager.notify:RegistListener(DORM_LOGIC_TICK, handler(slot0, slot0.Updata))
end

function slot0.RemoveEvent(slot0)
	manager.notify:RemoveListener(DORM_LOGIC_TICK, handler(slot0, slot0.Updata))
end
...
function slot0.WaitFoodBubble(slot0, slot1)
	...
	slot0.foodnmaetextText_.text = GetI18NText(BackHomeCanteenFoodCfg[slot0.foodID_].name)
	slot0.entityID = slot1.entityID
	slot0.tastenameText_.text = string.format("<color=#%s>%s</color>", CanteenConst.FoodTaste[slot2.taste][2], GetTips(CanteenConst.FoodTaste[slot2.taste][1]))
...
function slot0.Updata(slot0)
	if slot0.timer then
		if slot0.timebarImg_ then
			slot0.timebarImg_.fillAmount = 1 - slot0.timer:QueryTime() / slot0.time
		end
