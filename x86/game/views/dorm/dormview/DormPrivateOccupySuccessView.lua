...
function slot0.OnEnter(slot0)
	...
	if DormData:GetHeroInfo(slot0.archiveID):GetHeroId() then
		slot0.iconImg_.sprite = DormHeroTools.GetBackHomeHeadSprite(slot2)
	end

	if slot0.archiveID and HeroRecordCfg[slot0.archiveID] then
		if HeroRecordCfg[slot0.archiveID].name then
			slot0.nameTex_.text = string.format(GetTips("DORM_HERO_ROOM_NAME"), slot3)
		end
	else
