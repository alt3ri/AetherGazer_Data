...
function slot0.InitUI(slot0)
	...
	slot0.itemControllers_ = {
		...
		ControllerUtil.GetController(slot0.itemBtn5_.transform, slot4)
	}
	slot0.sealItemGos_ = {
...
function slot0.AddUIListener(slot0)
	...
	slot0:AddBtnListener(slot0.sealBtn_, nil, function ()
		...
		if ItemTools.getItemNum(slot0[1]) < slot0[2] then
			ShowTips(string.format(GetTips("CURRENCY_NO_ENOUGH"), tostring(ItemTools.getItemName(ItemCfg[slot0[1]].id))))
	...
	slot0:AddBtnListener(slot0.resetBtn_, slot4, function ()
		if uv0.playing_ then
			return
		end
...
function slot0.PlayOneCoordinateLine(slot0, slot1, slot2)
	...
	for slot8, slot9 in ipairs(slot1) do
		slot10 = XH1stSealData:DecodeCoordinateNumber(slot9)
...
function slot0.ShowResult(slot0, slot1, slot2)
	...
	slot6 = #slot1.special_reward_list > 0
	...
	for slot11, slot12 in ipairs(ActivitySealTicketCfg.get_id_list_by_ticket_id[slot2.ticket_id]) do
		if ActivitySealTicketCfg[slot12].row == slot1.coordinate.row and slot13.column == slot1.coordinate.column then
			slot0:UpdateView()
			getReward2(slot1.normal_reward_list, nil, function ()
				if #uv0.special_reward_list > 0 then
					uv1:PlayLinkEffect(uv2, function ()
						getReward2(uv0.special_reward_list)
...
function slot0.PlayDialog(slot0, slot1)
	...
	if slot3.sheetName[slot4] ~= "" then
		manager.audio:PlayEffect(slot6, slot7, slot8)

		slot0.talkBubbleTimer_ = TimeTools.StartAfterSeconds(manager.audio:GetVoiceLength(slot6, slot7, slot8) / 1000, function ()
			uv0:StopDialogTimer()
		end, {})
	end
end
