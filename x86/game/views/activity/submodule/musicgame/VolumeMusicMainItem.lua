...
function slot0.RefreshUI(slot0)
	...
	slot0.m_icon.sprite = getSpriteWithoutAtlas("TextureConfig/VolumeIIIDownUI/" .. slot4.icon)
end
	...
	if ActivityData:GetActivityData(slot0.music_id) then
		if slot5:IsActivitying() then
			if MusicData:GetScore(slot0.music_id, slot1) == 0 then
				...
				slot0.m_accuracyLab.text = string.format("%.2f%%", slot6 / slot4.total_score * 100)
			...
			if slot5 and manager.time:GetServerTime() < slot5.startTime then
