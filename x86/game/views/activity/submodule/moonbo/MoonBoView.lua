		...
		if uv0.isShowDice_ == false then
			...
			if ActivityData:GetActivityIsOpen(uv0.activityID_) then
				if MoonBoData:GetCanGetCount() <= 0 then
					...
					MoonBoAction:GetMoonBo(uv0.activityID_)
				end
			else
				uv0.isShowDice_ = false
...
function slot0.AddEventListeners(slot0)
	slot0:RegistEventListener(MOONBO_SUCCESS, function ()
		...
		AnimatorTools.PlayAnimationWithCallback(uv0.selfAni_, "VolumeBocakeMainUI_close", function ()
			uv0.isShowDiceController_:SetSelectedState("true")
			SetActive(uv0.diceGo_, true)

			uv0.todices_ = MoonBoData:GetCurDices()
			...
			uv0.activedice1_ = uv0:DiceToRight(uv0.dice1_, 1)
			uv0.activedice2_ = uv0:DiceToRight(uv0.dice2_, 2)
			uv0.activedice3_ = uv0:DiceToRight(uv0.dice3_, 3)
			uv0.activedice4_ = uv0:DiceToRight(uv0.dice4_, 4)
			uv0.activedice5_ = uv0:DiceToRight(uv0.dice5_, 5)
			uv0.activedice6_ = uv0:DiceToRight(uv0.dice6_, 6)
...
function slot0.OnEnter(slot0)
	...
	manager.redPoint:bindUIandKey(slot0.moonBoTrs_, string.format("%s_%s", RedPointConst.MOONBO, slot0.activityID_))
...
function slot0.OnExit(slot0)
	...
	manager.redPoint:unbindUIandKey(slot0.moonBoTrs_, string.format("%s_%s", RedPointConst.MOONBO, slot0.activityID_))
