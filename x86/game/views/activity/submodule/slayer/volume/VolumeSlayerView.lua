...
function slot0.SetSelectIndex(slot0, slot1)
	...
	if slot1 ~= 0 and not ActivityTools.GetActivityIsOpenWithTip(slot0.regionList[slot1], true) then
		return
	end
...
function slot0.Refresh(slot0)
	...
	for slot9, slot10 in ipairs(ActivitySlayerCfg[slot0.regionList[slot0.selectIndex]] and slot3.buff_desc or {}) do
		slot5 = GetI18NText("") .. GetI18NText(slot10) .. "\n"
	end

	slot0.m_buffDes.text = GetI18NText(slot5)
	slot7 = BattleSlayerStageCfg[slot3.stage_id]
	slot0.m_title.text = GetI18NText(slot7.name)
	slot0.m_des.text = GetI18NText(slot7.tips)
