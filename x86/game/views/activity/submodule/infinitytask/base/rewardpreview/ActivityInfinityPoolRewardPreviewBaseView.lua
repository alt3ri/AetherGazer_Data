...
function slot0.OnEnter(slot0)
	if OsirisInfinityPoolData:GetPoolData(slot0.params_.poolID).coreItem == nil then
		...
		SetActive(slot0.goMainItem_, true)

		for slot6 = 1, 1 do
			if slot0.mainRewardList_[slot6] == nil then
				slot0.mainRewardList_[slot6] = slot0:GetRewardPreviewItem().New(slot0.goItem_, slot0.goMainParent_, slot2.coreItem, slot2.isOpenCoreItem and 1 or 0, 1)
			else
	...
	for slot6 = 1, #slot2.resultItemList do
		if slot0.normalRewardList_[slot6] == nil then
			slot0.normalRewardList_[slot6] = slot0:GetRewardPreviewItem().New(slot0.goItem_, slot0.goNormalParent_, slot2.resultItemList[slot6].reward, slot7.cnt, slot7.maxCnt)
		else
