...
function slot0.RefreshUI(slot0)
	...
	if slot0.params_.point <= slot4 then
		...
		slot0.completeText_.text = GetI18NText(ActivityValentineCfg[slot0.params_.activityID].result_desc[1])
	elseif slot2 <= slot5 then
		slot0.resultText_.text = GetTips("ACTIVITY_VALENTINE_RESULT_TITLE_2")
		slot0.completeText_.text = GetI18NText(slot3.result_desc[2])
	else
		slot0.resultText_.text = GetTips("ACTIVITY_VALENTINE_RESULT_TITLE_3")
		slot0.completeText_.text = GetI18NText(slot3.result_desc[3])
	end
end
