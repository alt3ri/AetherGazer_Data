slot0 = class("SkuldTravelResultView", ReduxView)
...
function slot0.RefreshUI(slot0)
	slot1 = TravelSkuldStoryCfg[SkuldTravelData:GetCurPlotId()]

	for slot5 = 1, 4 do
		SetActive(slot0["attribgo_" .. slot5], false)
	end

	for slot5, slot6 in pairs(slot1.attrib_reward) do
		if slot6[2] > 0 then
			SetActive(slot0["attribgo_" .. slot6[1]], true)

			slot0["attribtext_" .. slot6[1]].text = GetI18NText(slot6[2])
		end
	end
end
