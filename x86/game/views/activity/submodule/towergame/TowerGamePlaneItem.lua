...
function slot0.SetData(slot0, slot1, slot2)
	...
	slot0.nametext_.text = GetI18NText(slot0.cfg.name)
	slot0.desctext_.text = GetI18NText(slot0.cfg.desc)
	...
	if TowerGameData:GetPlaneIsUnlock(slot0.cfg.id) then
		...
		slot0.locktext_.text = string.format(GetTips("ACTIVITY_TOWER_DEFENSE_UNLOCK_DESC"), GetI18NText(BattleTowerGameCfg[TowerGameCfg[TowerGamePlaneCfg[slot0.cfg.id].condition].stage_id].name))
	end
end
