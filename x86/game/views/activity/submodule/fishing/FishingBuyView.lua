...
function slot0.AddUIListener(slot0)
	slot0.slider_:SetNumChangeHandler(function (slot0)
		uv0:UpdateView()
	end)
	...
	slot0:AddBtnListener(slot0.okBtn_, nil, function ()
		if ItemTools.getItemNum(CurrencyConst.CURRENCY_TYPE_SUMMER_FISHING_COIN) < uv0.cfg_.price * uv0.slider_:GetSelectedNum() then
...
function slot0.UpdateView(slot0)
	...
	if slot0.cfg_.price ~= nil and slot0.cfg_.price ~= 0 then
		slot0.maxNum_ = math.floor(ItemTools.getItemNum(CurrencyConst.CURRENCY_TYPE_SUMMER_FISHING_COIN) / slot0.cfg_.price)
	end
end
...
function slot0.OnEnter(slot0)
	...
	slot0.icon2_.sprite = getSpriteWithoutAtlas(string.format("TextureConfig/Fishing/RareFishIcon/%d", slot0.params_.fishId))
	slot0.useNameText_.text = ItemTools.getItemName(CurrencyConst.CURRENCY_TYPE_SUMMER_FISHING_COIN)
	slot0.getNameText_.text = GetI18NText(RareFishCfg[slot0.params_.fishId].name)
