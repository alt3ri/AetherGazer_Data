...
function slot0.OnEnter(slot0)
	...
	for slot5, slot6 in ipairs(slot0.activityIDList_) do
		manager.redPoint:bindUIandKey(slot0[string.format("transformRedPoint%s_", slot5)], ActivityTools.GetRedPointKey(slot6) .. slot6)
	end
...
function slot0.OnExit(slot0)
	...
	for slot4, slot5 in ipairs(slot0.activityIDList_) do
		manager.redPoint:unbindUIandKey(slot0[string.format("transformRedPoint%s_", slot4)], ActivityTools.GetRedPointKey(slot5) .. slot5)
	end
...
function slot0.AddListeners(slot0)
	slot0:AddBtnListener(slot0.btnSwitch_, nil, function ()
		for slot4, slot5 in pairs(uv0.activityIDList_) do
			if ActivityStoryChapterCfg[uv0.selectChapterID_].activity_id ~= slot5 then
...
function slot0.RefreshSwitchBtn(slot0)
	if ActivityStoryChapterCfg[slot0.selectChapterID_].activity_id == slot0.activityIDList_[1] then
		...
		slot0.upChapter_.text = GetTips("FRONT_CHAPTER")
		slot0.downLockChapter_.text = GetTips("AFTER_CHAPTER")
		slot0.downUnLockChapter_.text = GetTips("AFTER_CHAPTER")
	else
		slot0.switchController_:SetSelectedState("2")

		slot0.upChapter_.text = GetTips("AFTER_CHAPTER")
		slot0.downLockChapter_.text = GetTips("FRONT_CHAPTER")
		slot0.downUnLockChapter_.text = GetTips("FRONT_CHAPTER")
	end

	for slot6, slot7 in pairs(slot0.activityIDList_) do
		if ActivityStoryChapterCfg[slot0.selectChapterID_].activity_id ~= slot7 then
...
function slot0.AddTimer(slot0)
	...
	for slot7, slot8 in pairs(slot0.activityIDList_) do
		if ActivityStoryChapterCfg[slot0.selectChapterID_].activity_id ~= slot8 then
			slot1 = slot8
...
function slot0.RefreshStageList(slot0)
	...
	for slot5 = 1, table.length(StoryStageActivityData:GetStageData(slot0.selectChapterID_)) do
...
function slot0.RefreshSelectItem(slot0)
	for slot5, slot6 in pairs(slot0.stageItemList_) do
		slot6:SelectorItem(ActivityStoryChapterCfg[slot0.selectChapterID_].stage_list[slot0.selectStageIndex_])
	end
end
