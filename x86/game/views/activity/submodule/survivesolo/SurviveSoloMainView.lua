...
function slot0.InitUI(slot0)
	...
	for slot4, slot5 in ipairs(slot0.activityList_) do
		...
		slot0.levelText_[slot4].text = GetI18NText(BattleSoloSlayerCfg[ActivitySoloSlayerCfg[slot5].stage_id].name)
...
function slot0.SetIndex(slot0, slot1)
	...
	if slot1 ~= 0 and slot1 ~= -1 then
		...
		manager.redPoint:setTip(RedPointConst.SOLO_SURVIVE_OPEN .. "_" .. slot2, 0)
...
function slot0.BindUIRedPoint(slot0)
	slot5 = "_"
	slot4 = RedPointConst.SOLO_SURVIVE_REWARD .. slot5 .. slot0.activityID_

	manager.redPoint:bindUIandKey(slot0.rewardBtn_.gameObject.transform, slot4)
...
function slot0.UnbindUIRedPoint(slot0)
	...
	manager.redPoint:unbindUIandKey(slot0.rewardBtn_.gameObject.transform, slot4)
