slot0 = class("NewServerSupplyPage", ReduxView)

function slot0.OnCtor(slot0, slot1)
	slot3 = Object.Instantiate(Asset.Load("Widget/System/NewServer/NewServerShopUI"), slot1)
...
function slot0.SetData(slot0)
	slot0.curShopId_ = ShopConst.SHOP_ID.NEW_SERVER_SHOP
...
function slot0.InitShopList(slot0, slot1)
	...
	for slot7, slot8 in ipairs(ShopTools.FilterShopDataList(slot1)) do
		if RechargeShopDescriptionCfg[getShopCfg(slot8.id).description] ~= nil then
	...
	table.sort(slot2, function (slot0, slot1)
		...
		if uv0:GetGoodStatus(slot0.id, uv1) ~= uv0:GetGoodStatus(slot1.id, uv1) then
			return slot4 < slot5
		end
...
function slot0.GetGoodStatus(slot0, slot1, slot2)
	slot3 = false
	slot6 = ShopData.GetShop(getShopCfg(slot1, slot2).shop_id)[slot1]
...
function slot0.RefreshShopGoodInfo(slot0, slot1, slot2)
	...
	for slot6, slot7 in pairs(slot0.giftList_:GetItemList()) do
