...
function slot0.OnShow(slot0, slot1)
	...
	slot3 = nil
	slot0.skinnedMeshRenderer_ = (not slot0:FindTrs("Face", slot0.uiTpose_.transform) or slot0:FindCom(typeof(UnityEngine.SkinnedMeshRenderer), "Face", slot0.uiTpose_.transform)) and slot0:FindCom(typeof(UnityEngine.SkinnedMeshRenderer), "face", slot0.uiTpose_.transform)

	for slot8, slot9 in ipairs(HeroDisplayCfg[slot0.heroID_].skinMeshIDList) do
		...
		slot0.itemList_[slot8]:SetData(slot0.skinnedMeshRenderer_, slot8, slot9)
	end

	for slot8 = #slot4.skinMeshIDList + 1, #slot0.itemList_ do
		slot0.itemList_[slot8]:Show(false)
	end

	if HeroDisplayData:GetCacheExpressionParams().values[1] then
		for slot10, slot11 in ipairs(slot6) do
			slot0.itemList_[slot10]:SetValue(slot11)
		end
	else
...
function slot0.InitExpressionDropdown(slot0)
	...
	slot8 = GetTips("SAVE_NEW_PRESET")

	slot0.dropdownItemList_[#slot3 + 1]:SetData(#slot3 + 1, slot8)
	...
	slot0.textPreset_.text = GetI18NText(slot3[slot0.lastPresetInext_].name)
	slot8 = GetTips("NONE")

	slot0.dropDownEffect_.options:Add(UnityEngine.UI.Dropdown.OptionData.New(slot8))
...
function slot0.RefreshItemValue(slot0, slot1)
	...
	slot0.textPreset_.text = GetI18NText(slot4.name)
...
function slot0.TrySwitchIndex(slot0, slot1)
	...
	JumpTools.OpenPageByJump("threeButtonBox", {
		...
		okFun = function ()
			slot0 = {
				preset = false,
				values = clone(slot2:GetCacheExpressionParams().values),
...
function slot0.TrySaveData(slot0)
	slot1 = slot0.heroID_
	slot2 = HeroDisplayData:GetCacheExpressionParams()
	slot3 = {
		preset = false,
...
function slot0.TryExitSaveData(slot0)
	...
	JumpTools.OpenPageByJump("threeButtonBox", {
		...
		okFun = function ()
			slot0 = uv0.heroID_
			slot1 = HeroDisplayData:GetCacheExpressionParams()
			slot2 = {
				preset = false,
...
function slot0.SwitchSaveData(slot0, slot1, slot2)
	...
	for slot7, slot8 in ipairs(HeroDisplayData:GetHeroExpressionParams(slot7)) do
