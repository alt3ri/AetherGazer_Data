...
function slot0.UIName(slot0, slot1)
	return CultivateHeroTools.GetMainUIName(slot1)
end
	...
	slot0:AddBtnListener(slot0.receiveDailyBtn_, nil, function ()
		...
		if uv0.canReceiveDaily_ == true then
			...
			JumpTools.OpenPageByJump("rewardPreview", {
				rewardList = slot0,
				extraItemTemplateDataList = slot1
...
function slot0.RefreshUI(slot0)
	...
	slot0.heroLockState_:SetSelectedState(tostring(not HeroTools.GetHeroIsUnlock(CultivateHeroTaskCfg[CultivateHeroTaskCfg.get_id_list_by_group_id[slot1][1]].hero)))
...
function slot0.GetSwitchItemClass(slot0)
	return CultivateHeroTools.GetSwitchItemClass(slot0.activityID_)
end
...
function slot0.OnClickSwitchItem(slot0, slot1)
	if slot0.selectHeroID_ == slot1 then
		return
	end
...
function slot0.RefreshHeroTask(slot0)
	...
	slot0.heroNameText_.text = string.format("%s·%s", GetI18NText(HeroCfg[slot2].name), GetI18NText(HeroCfg[slot2].suffix))
...
function slot0.RefreshAccumulateTask(slot0)
	slot1 = ActivityPointRewardCfg.get_id_list_by_activity_id[slot0.activityID_]
	...
	slot13 = slot7.need
	slot0.curAccumulateProgressText_.text = string.format(GetTips("VERIFY_ASSETS_PROCESSING_RATE"), slot2, slot13)
	slot0.coinImage_.sprite = ItemTools.getItemSprite(ActivityCultivateHeroCfg[slot0.activityID_].coin_id)

	for slot13, slot14 in ipairs(slot7.reward_item_list) do
		if not slot0.accumulateRewardList_[slot13] then
			slot0.accumulateRewardList_[slot13] = CommonItemPool.New(slot0.goRewardPanel_, nil, true)
			slot0.itemDataList_[slot13] = clone(ItemTemplateData)

			slot0.itemDataList_[slot13].clickFun = function (slot0)
		...
		slot0.itemDataList_[slot13].id = slot14[1]
		slot0.itemDataList_[slot13].number = slot14[2]

		slot0.accumulateRewardList_[slot13]:Show(true)
		slot0.accumulateRewardList_[slot13]:SetData(slot0.itemDataList_[slot13])
	end
...
function slot0.RefreshRoleImage(slot0)
	slot0.roleImage_.sprite = getSpriteWithoutAtlas("TextureConfig/Character/Portrait/" .. CultivateHeroTaskCfg[CultivateHeroTaskCfg.get_id_list_by_group_id[slot0.selectHeroID_][1]].hero)
end
...
function slot0.AddTimer(slot0)
	...
	slot0.textTime_.text = manager.time:GetLostTimeStrWith2Unit(slot0.stopTime_)
	slot0.timer_ = Timer.New(function ()
		...
		uv0.textTime_.text = manager.time:GetLostTimeStrWith2Unit(uv0.stopTime_)
	end, 1, -1)
