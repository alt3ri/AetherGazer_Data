...
function slot0.InitUI(slot0)
	...
	slot5 = slot0.ItemRander

	slot0.tree_:SetRenderer(handler(slot0, slot5))
...
function slot0.OnEnter(slot0)
	if slot0.isInit_ then
		...
		slot0.timer_ = Timer.New(function ()
			for slot3 = 1, ArtifactLevelData:GetLevelCount() do
...
function slot0.AwardIndexItem(slot0, slot1, slot2)
	slot2:RefreshData(rewardToItemTemplate(slot0.curAwardsList_[slot1]))

	if ArtifactLevelData:GetIsAwardedByIndex(slot0.curLevelIndex_) then
		ControllerUtil.GetController(slot2.transform_, "received"):SetSelectedState("true")
	else
...
function slot0.RegisterRedPoint(slot0)
	for slot4 = 1, ArtifactLevelData:GetLevelCount() do
