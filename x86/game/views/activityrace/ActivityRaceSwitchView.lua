...
function slot0.AddUIListener(slot0)
	slot0:AddBtnListener(slot0.shopBtn_, nil, function ()
		...
		for slot6, slot7 in ipairs(ActivityShopCfg.get_id_list_by_activity_theme[ActivityShopCfg[ActivityRaceCfg[uv0.activityID_].main_activity_id].activity_theme]) do
...
function slot0.RefreshTitle(slot0)
	if SDKTools.GetIsOverSea() then
		slot1 = ActivityRaceCfg[slot0.activityID_].map_title_icon .. "@" .. SettingData:GetCurrentLanguage()
	end

	slot0.titleIcon_.sprite = getSprite("Atlas/VolumeIIIDownUI", slot1)

	slot0.titleIcon_:SetNativeSize()
...
function slot0.RefreshAffixItem(slot0)
	...
	slot0.unlockAffixList_ = ActivityRaceData:GetUnlockAffixList(slot0.activityID_)
...
function slot0.RefreshRaceBuff(slot0)
	slot0.raceBuffDesc.text = GetI18NText(getAffixDesc(ActivityAffixPoolCfg[ActivityRaceCfg[slot0.activityID_].race_buff].affix))
end
...
function slot0.OnSelectAffixItem(slot0, slot1, slot2, slot3)
	...
	slot0.affixcontentText_.text = GetI18NText(GetCfgDescription(slot4.description[1], slot3))
	slot0.affixnameText_.text = GetI18NText(slot4.name)
