	...
	for slot5, slot6 in ipairs(slot0.equipInfo_) do
		if slot0.extra_ then
			...
			elseif slot0.isTemp_ then
				if slot7 ~= 0 then
					slot0.itemS_[slot5]:RefreshData(slot0, TempHeroData:GetEquipInfoList(slot0.heroInfo_.id, slot0.extra_.tempHeroList)[slot5])
				else
					slot0.itemS_[slot5]:RefreshData(slot0, {
						equip_id = 0,
						pos = slot5
			...
			elseif slot0.isPolyhedron_ then
				if slot7 ~= 0 then
					slot9 = PolyhedronHeroCfg[slot0.heroInfo_.id].standard_id

					slot0.itemS_[slot5]:RefreshData(slot0, slot0.heroInfo_.equip_list[slot5])
				else
					slot0.itemS_[slot5]:RefreshData(slot0, {
						equip_id = 0,
						pos = slot5
			...
			elseif slot0.isCooperation_ then
				if slot7 ~= 0 then
					slot0.itemS_[slot5]:RefreshData(slot0, CooperationTools.GetEquipInfo(slot0.heroInfo_.id, slot5))
				else
					slot0.itemS_[slot5]:RefreshData(slot0, {
						equip_id = 0,
						pos = slot5
			...
			elseif slot0.isAdvanceTest_ then
				if slot7 ~= 0 then
