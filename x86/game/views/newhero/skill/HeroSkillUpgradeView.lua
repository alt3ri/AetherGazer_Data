...
function slot0.GetRealSkillId(slot0)
	if slot0.heroViewProxy_:GetViewDataType() == HeroConst.HERO_DATA_TYPE.PREVIEW then
		...
		return slot0.heroViewProxy_:GetRealSkillId(slot0.params_.heroId, slot0.params_.skillId)
	end
end
...
function slot0.RefreshSkill(slot0)
	...
	slot0.subnameText_.text = GetI18NText(slot2.skill_subhead[table.indexof(slot2.skills, slot0.params_.skillId)])
	slot0.typeText_.text = GetI18NText(SkillElementCfg[slot1.element_type[1]].name)
	...
	slot0.textSkillType_.text = GetI18NText(SkillSubTypeCfg[SkillSubTypeCfg.get_id_list_by_value[slot6][1]].annotation)
end

function slot0.GetSkillLv(slot0)
	if not slot0.skillLevel_ or slot0.skillLevel_ == 0 then
		slot0.skillLevel_ = slot0.heroViewProxy_:GetSkillLv(slot0.params_.heroId, slot0.params_.skillId)
	end
...
function slot0.RefreshMessage(slot0)
	slot1 = slot0.heroViewProxy_:GetHeroData(slot0.params_.heroId)
	slot0.level_ = slot0:GetSkillLv()
	slot0.addEquipSkillLv_ = 0
	slot0.addEquipSkillLv_ = slot0.heroViewProxy_:GetTransitionAddLevel(slot1.id, slot0.id_)
...
function slot0.RefreshDesc(slot0, slot1)
	if slot1 == 1 then
		slot0.contentText_.text = GetI18NText(HeroSkillCfg[slot0.realSkillId_].simpleDesc)
	else
		slot0.contentText_.text = SkillTools.GetSkillDesc(slot0.realSkillId_, slot0.level_ + slot0.addSkillLv_ + slot0.addEquipSkillLv_, SkillTools.GetIsDodgeSkill(slot0.params_.skillId), slot0.extra_)
	end
...
function slot0.RefreshCost(slot0)
	if not slot0.isPreview_ then
		...
		if slot0.tempAddLevel_ and slot0.tempAddLevel_ > 0 then
			slot2 = ItemTools.getItemNum(slot0.costCfg_[1]) - SkillTools.GetSkillUpCostNum(slot0.params_.skillId, slot0.skillLevel_, slot0.tempAddLevel_)[slot0.costCfg_[1]]
		end

		if slot2 < slot0.costCfg_[2] then
			slot0.costEnough_ = false
			slot0.costNumText_.text = string.format("<color=#F43950>%d</color>/%d", slot2, slot0.costCfg_[2])
		else
			slot0.costEnough_ = true
			slot0.costNumText_.text = string.format("%d/%d", slot2, slot0.costCfg_[2])
		end
	end
end
