...
function slot0.PlayInteractionAnimation(slot0, slot1)
	...
	if HeroInteractionCfg[SkinCfg[slot0.interactiveSkinId_].hero] and slot3[slot1 .. "_talk"] ~= nil and #slot3[slot1 .. "_talk"] > 0 then
		if #slot3[slot1 .. "_talk"] == 1 then
			slot0:PlaySpecialAnimation(#slot3[slot1] > 0 and slot3[slot1][1] or nil, slot3[slot1 .. "_talk"][1], slot3[slot1 .. "_delay"][1])
		else
...
function slot0.RandomIndexOfInteractionType(slot0, slot1)
	...
	for slot11, slot12 in ipairs(HeroInteractionCfg[SkinCfg[slot0.interactiveSkinId_].hero][slot1]) do
		if slot11 ~= (slot0.lastRandom_[slot2] or 0) then
			table.insert({}, slot12)
