...
function slot1.SetHeroData(slot0, slot1, slot2, slot3)
	...
	slot0.heroInfo_ = PolyhedronData:GetHeroData(slot1)
	slot0.heroDataType_ = HeroConst.HERO_DATA_TYPE.POLYHEDRON

	slot0:UpdateView()

	slot0.headIcon_.sprite = HeroTools.GetHeadSprite(PolyhedronData:GetHeroUsingSkinInfo(slot0.heroId_).id)

	if not PolyhedronData:GetPolyhedronHeroUnlock(slot0.heroId_) and slot0.heroInfo_.unlock == 1 and ItemTools.getItemNum(CurrencyConst.CURRENCY_TYPE_UNLOCK_PROPS) > 0 then
