slot0 = class("ActivityNewbieUpgradeView", ReduxView)
...
function slot0.AddListeners(slot0)
	slot0:AddBtnListener(slot0.button_, nil, function ()
		...
		if uv0.isFinish_ and not table.keyof(ActivityNewbieData:GetUpgradeFinishList(), slot1) then
			ActivityNewbieAction.ReceiveUpgradeReward(slot1, function (slot0)
				if isSuccess(slot0.result) then
					getReward2(slot0.reward_list)
					ActivityNewbieData:SetUpgradeFinish(uv0)
...
function slot0.RefreshUI(slot0)
	...
	if slot0.isFinish_ then
		if table.keyof(ActivityNewbieData:GetUpgradeFinishList(), slot2) then
			slot0:ReceiveReward()
		else
