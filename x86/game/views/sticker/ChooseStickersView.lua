...
function slot0.GetAllSticker(slot0)
	for slot4, slot5 in ipairs(PlayerData:GetStickerList()) do
...
function slot0.SetCurID(slot0, slot1)
	...
	slot0.nameTxt_.text = ItemTools.getItemName(slot3.id)
	slot0.descTxt_.text = ItemTools.getItemDesc(slot3.id)
	slot0.wayTxt_.text = GetI18NText(slot3.desc_source) or ""
end
...
function slot0.RefreshList(slot0)
	for slot4, slot5 in pairs(slot0.scrollList_:GetItemList()) do
...
function slot0.RefreshSize(slot0)
	for slot6 = 1, 3 do
		if not table.indexof(StickViewTemplateCfg[slot0.viewID_].range_list[slot0.curIndex_][2], slot6) then
