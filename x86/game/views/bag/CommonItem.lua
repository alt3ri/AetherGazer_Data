...
function slot0.InitUI(slot0)
	...
	slot0.ownedController_ = ControllerUtil.GetController(slot0.gameObject_.transform, "owned")
end
...
function slot0.UpdateType(slot0, slot1)
	slot0.typeController_:SetSelectedState("reset")
...
function slot0.UpdateMaterialView(slot0)
	...
	if slot0.info_.num then
		slot0:SetBottomText(slot0.info_.num)
	end
end
...
function slot0.UpdateWeaponServantView(slot0)
	...
	if slot0.info_.isAdd then
		slot0.servantStageText_.text = "<color=#FF9500>" .. tostring(slot0.info_.num) .. "</color>"

		slot0:SetBottomText("")
	else
		slot0.servantStageText_.text = ""
	...
	if ItemCfg[slot0.info_.id].type == ItemConst.ITEM_TYPE.WEAPON_SERVANT then
		slot0.servantStageText_.text = " "
	...
	if slot0.info_.equiping and slot0.info_.equiping.heroId > 0 then
		slot2 = true
		slot3 = slot0.info_.equiping.heroId
	end

	slot0:ShowEquiping(slot2, slot3)
	slot0:UpdateCamp(WeaponServantCfg[slot0.info_.id].race)
	slot0:ShowLock(slot0.info_.locked == 1)
end

function slot0.UpdateEquipView(slot0)
	slot0:UpdateLevel(slot0.info_.level or slot0.info_.num or 1)
	...
	if (slot0.info_.equip_info and (slot0.info_.equip_info.race or 0) or slot0.info_.race or 0) == 0 or table.keyof(RaceEffectCfg.all, slot5) ~= nil then
...
function slot0.UpdateHeroView(slot0)
	slot0:SetBottomText(slot0.info_.num or 1)
...
function slot0.UpdateName(slot0)
	if ItemCfg[slot0.info_.id] then
		slot0.nameText_.text = ItemTools.getItemName(slot0.info_.id)
	end
end
...
function slot0.ShowOwned(slot0, slot1)
	if not slot0.ownedController_ then
		return
	end

	slot0.ownedController_:SetSelectedState(slot1 or slot0.info_.owned and "true" or "false")
end
...
function slot0.ShowStar(slot0, slot1)
	if slot1 then
		slot0:UpdateStar()
	else
		slot0.starController_:SetSelectedState("0")
	end
end
...
function slot0.UpdateTime(slot0)
	...
	if ItemTools.GetItemExpiredTimeByInfo(slot0.info_) == nil or slot3 == 0 then
		slot0.countdownStatusController_:SetSelectedState("noLimit")
	elseif ItemTools.IsItemExpired(slot0.info_) then
		slot1:SetSelectedState("expired")
	elseif slot0.info_.timeValid then
