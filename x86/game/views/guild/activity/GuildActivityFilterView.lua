slot0 = class("GuildActivityFilterView", ReduxView)
...
function slot0.InitDropDown(slot0)
	...
	slot0.sortTypeDropdown_.options:Add(UnityEngine.UI.Dropdown.OptionData.New(GetTips("TIP_UPGRADE"), nil))
	slot0.sortTypeDropdown_.options:Add(UnityEngine.UI.Dropdown.OptionData.New(GetTips("TIP_RARE"), nil))
	slot0.sortTypeDropdown_:RefreshShownValue()
end
...
function slot0.RefreshSort(slot0, slot1, slot2, slot3, slot4, slot5)
	...
	if slot0.isAll_ then
		...
		for slot9, slot10 in ipairs(slot0.heroList_) do
			...
			if #slot0.curAttackType_ <= 0 or #slot12.ATK_attribute > 1 and table.indexof(slot0.curAttackType_, HeroConst.HERO_ATTACK_TYPE.MIX) then
				slot15 = true
			else
			...
			if slot13 and slot14 and slot15 then
				table.insert(slot0.displayHeroList_, slot11)
			end
		end
	end
...
function slot0.UpdateList(slot0)
	...
	table.sort(slot0.displayHeroList_, function (slot0, slot1)
		...
		if HeroData:IsFavorite(slot4) ~= HeroData:IsFavorite(slot5) and (not slot8 or not slot9) then
			return slot8 ~= false
		end

		if uv0.curSortType_ == 0 then
			slot10, slot3 = uv0:EnergySorter(slot6, slot7)

			if slot10 then
				return slot3
			end

			slot10, slot3 = uv0:FightPowerSorter(slot6, slot7)

			if slot10 then
				return slot3
			end

			slot10, slot3 = uv0:StarSorter(slot6, slot7)

			if slot10 then
				return slot3
			end

			slot10, slot3 = uv0:RareSorter(slot6, slot7)

			if slot10 then
				return slot3
			end

			slot10, slot3 = uv0:LevelSorter(slot6, slot7)
...
function slot0.EnergySorter(slot0, slot1, slot2)
	if GuildActivityData:GetHeroFatigue(slot1.id) ~= GuildActivityData:GetHeroFatigue(slot2.id) then
...
function slot0.ShowSortBtn(slot0, slot1)
	SetActive(slot0.sortBtnGo_, slot1)
end
