slot0 = class("GuildActivitySPFightHeroSettingView", ReduxView)
...
function slot0.InitUI(slot0)
	...
	slot0.headList_ = LuaList.New(handler(slot0, slot0.indexItem), slot0.uiListGo_, GuildActivitySPFightHeadItemView)
end
...
function slot0.AddUIListener(slot0)
	...
	slot0:AddBtnListener(slot0.okBtn_, nil, function ()
		if GuildActivitySPData:IsFirstConfigFightRole() and #uv0.heroDataList_ < GuildActivitySPData:GetMaxFightHeroCount() then
			ShowMessageBox({
...
function slot0.StartEdit(slot0)
	...
	slot0.headList_:StartScroll(ActivityClubSPLevelSettingCfg[ActivityClubSPLevelSettingCfg.all[#ActivityClubSPLevelSettingCfg.all]].max_hero)
end

function slot0.SwitchToList(slot0)
	slot0.putOffHeroList_ = {}
	slot0.heroDataList_ = GuildActivitySPData:GetFightHeroList()
...
function slot0.SaveHeroes(slot0)
	...
	GuildActivitySPAction.SetFightMember(slot1, slot2)
	slot0:SwitchToList()
end

function slot0.AddEventListeners(slot0)
	slot0:RegistEventListener(GUILD_ACTIVITY_SP_HERO_LIST_UPDATE, function ()
		uv0:SwitchToList()
	end)
end
...
function slot0.OnEnter(slot0)
	...
	slot0.hoursLabel_.text = string.format(GetTips("ACTIVITY_CLUB_VITALITY_RECOVER"), tostring(60 / GameSetting.activity_club_sp_hero_fatigue_recovery.value[1]))
end
