slot0 = class("GuildActivitySPAffixMainView", ReduxView)
...
function slot0.InitUI(slot0)
	...
	for slot5 = 1, slot0.racePanelTrans_.childCount do
		slot0.raceItemList_[slot5] = GuildActivitySPAffixRaceItem.New(slot0.racePanelTrans_:GetChild(slot5 - 1))

		slot0.raceItemList_[slot5]:SetSelectCallBack(slot0.selectRaceItemHandler_)
	end

	for slot6 = 1, slot0.affixPanelTrans_.childCount do
		slot0.affixItemList_[slot6] = GuildActivitySPAffixItem.New(slot0.affixPanelTrans_:GetChild(slot6 - 1))
	...
	for slot7 = 1, slot0.affixInfoPanelTrans_.childCount do
		slot0.affixInfoItemList_[slot7] = GuildActivitySPAffixInfoItem.New(slot0.affixInfoPanelTrans_:GetChild(slot7 - 1))
	end
end
...
function slot0.AddUIListener(slot0)
	slot0:AddBtnListener(slot0.upgradeBtn_, nil, function ()
		...
		GuildActivitySPAction.UpgradeAffix(uv0.curAffixID_, function (slot0, slot1)
			if isSuccess(slot0.result) then
				manager.notify:Invoke(MATERIAL_MODIFY, uv0.currencyID_)

				slot5 = uv0.curAffixID_

				GuildActivitySPData:UpgradeAffix(uv0.activityID_, slot5)
				...
				slot2, slot3 = GuildActivitySPData:CheckRateUpgrade(uv0.activityID_)

				if slot2 == true then
					JumpTools.OpenPageByJump("guildActivitySPRateUpgrad", {
						rateID = slot3
					})
					GuildActivitySPData:RefreshRateRedPoint(uv0.params_.activityID)
				end
			else
...
function slot0.InitBar(slot0)
	...
	manager.windowBar:SetGameHelpKey(GuildActivitySpTools.GetCurOpenActivityStateInfo().helpKey)
end
...
function slot0.RefreshRaceItem(slot0)
	...
	slot0.curRaceID_ = GuildActivitySPData:GetSelectRaceID(slot0.activityID_) or slot0.raceIDList_[1]

	slot0:OnSelectRaceItem(slot0.curRaceID_)
end

function slot0.OnSelectRaceItem(slot0, slot1)
	GuildActivitySPData:SetSelectRaceID(slot0.activityID_, slot1)
...
function slot0.RefreshAffixItem(slot0)
	...
	slot0.unlockAffixIDList_ = GuildActivitySPData:GetUnLockAffixList()
	...
	slot4 = GuildActivitySPData:GetSelectAffixID(slot0.activityID_, slot0.curRaceID_)
	slot0.curAffixID_ = GuildActivitySPData:GetSelectAffixID(slot0.activityID_, slot0.curRaceID_) or slot0.affixIDList_[1]
...
function slot0.OnSelectAffixItem(slot0, slot1, slot2)
	slot6 = slot0.curRaceID_
	slot7 = slot1

	GuildActivitySPData:SetSelectAffixID(slot0.activityID_, slot6, slot7)
