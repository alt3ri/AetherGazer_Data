...
function slot0.RefreshEquipBTMaterialHP(slot0)
	...
	if #EquipBreakThroughMaterialData:GetHeroState() > 0 then
		...
		for slot6, slot7 in pairs(EquipBreakThroughMaterialData:GetCacheHero()) do
			slot2[slot6] = {
				rate = 10000,
				heroID = slot7
			}
	...
	for slot6, slot7 in pairs(slot2) do
		if slot7.heroID ~= 0 then
			...
			if slot7.rate > 0 and math.ceil(slot7.rate / 100) < 1 then
				slot8 = 1
			end
...
function slot0.StartBattle(slot0)
	if EquipBreakThroughMaterialData:GetComboSkillID() == -1 then
		EquipBreakThroughMaterialAction.SaveComboSkillID(ComboSkillData:GetComboSkillID(slot0.stageType_, slot0.activityID_, 1), function ()
