...
function slot0.SetData(slot0, slot1, slot2, slot3)
	slot0.desText_.text = GetI18NText(ConditionCfg[slot1].desc)

	if ConditionCfg[slot1].type == 5001 then
		slot4 = HeroRecordCfg[HeroRecordCfg.get_id_list_by_hero_id[slot2][1]]
		slot0.desText_.text = string.format(ConditionCfg[slot1].desc, StoryCfg[slot4.plot_id[#slot4.plot_id]].name)
	end
