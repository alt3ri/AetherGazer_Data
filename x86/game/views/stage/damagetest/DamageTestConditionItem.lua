slot0 = class("DamageTestConditionItem", ReduxView)
...
function slot0.AddListeners(slot0)
	slot0:AddBtnListener(slot0.button_, nil, function ()
		DamageTestData:SetSelectAffix(uv0.damageTestId_, uv0.itemType_, uv0.id_)
...
function slot0.RefreshMutexStage(slot0)
	...
	for slot6, slot7 in ipairs(DamageTestData:GetAffixList(slot0.damageTestId_, slot0.itemType_)) do
		if slot0.id_ ~= slot7 and slot1 and slot1.condition[1] == StageChallengeConditionPoolCfg[slot7].condition[1] then
...
function slot0.RefreshMutexAffix(slot0)
	...
	for slot6, slot7 in ipairs(DamageTestData:GetAffixList(slot0.damageTestId_, slot0.itemType_)) do
		if slot0.id_ ~= slot7 and slot1 and slot1.affix[1] == ActivityAffixPoolCfg[slot7].affix[1] then
			slot0.controller_:SetSelectedState("not")
