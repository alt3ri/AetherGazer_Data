...
function slot0.RefreshMutexStage(slot0)
	for slot6, slot7 in ipairs(BattleBossChallengeAdvanceData:GetBossAffixList(slot0.bossIndex_, slot0.itemType_)) do
		if slot0.id_ ~= slot7 and StageChallengeConditionPoolCfg[slot0.id_].condition[1] == StageChallengeConditionPoolCfg[slot7].condition[1] then
...
function slot0.RefreshMutexAffix(slot0)
	for slot6, slot7 in ipairs(BattleBossChallengeAdvanceData:GetBossAffixList(slot0.bossIndex_, slot0.itemType_)) do
		if slot0.id_ ~= slot7 and ActivityAffixPoolCfg[slot0.id_].affix[1] == ActivityAffixPoolCfg[slot7].affix[1] then
			slot0.controller_:SetSelectedState("not")
