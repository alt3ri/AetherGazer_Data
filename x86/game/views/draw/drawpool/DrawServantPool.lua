...
function slot0.AddUIListener(slot0)
	...
	slot0:AddBtnListener(slot0.btnlook1Btn_, nil, function ()
		if table.indexof(DrawPoolCfg[uv0.poolId].optional_lists_poolId, uv0.showId) and slot0.optional_detail[slot1] then
...
function slot0.SetActive(slot0, slot1)
	...
	if slot1 == true then
		manager.redPoint:bindUIandKey(slot0.btnshow_.transform, string.format("%s_%d_%d", RedPointConst.WEAPON_SERVANT_POOL, slot0.poolId, DrawData:GetPollUpID(slot0.poolId)))
	end
end
