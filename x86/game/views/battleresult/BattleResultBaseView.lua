...
function slot0.AddListener(slot0)
	...
	slot0:AddBtnListener(slot0.btnOnceMore_, nil, function ()
		...
		if ItemTools.getItemNum(CurrencyConst.CURRENCY_TYPE_VITALITY) < uv0.stageData:GetCost() * uv0.stageData:GetMultiple() then
...
function slot0.SetLevelTitle(slot0)
	...
	slot0.lvText_.text = GetI18NText(BattleStageTools.GetStageName(slot0.stageType, slot0.stageId))
...
function slot0.RefreshRewardItemS(slot0)
	...
	for slot5 = 1, #slot1 do
		...
		if ItemCfg[slot1[slot5].id] and ItemConst.ITEM_TYPE.EQUIP == ItemCfg[slot6.id].type then
			slot6.race = EquipCfg[slot6.id] and EquipCfg[slot6.id].race
		end

		slot0.rewardItems_[slot5]:RefreshData(rewardToItemTemplate(slot6))
	end
end
...
function slot0.RefreshHeroS(slot0)
	...
	for slot8 = 1, 3 do
		if not slot1[slot8] or slot1[slot8] == 0 then
			SetActive(slot0.heroGo_[slot8], false)
		else
			SetActive(slot0.heroGo_[slot8], true)

			if slot0.heroImg_[slot8] ~= nil then
				slot0.heroImg_[slot8].sprite = getSpriteViaConfig("HeroIcon", SkinCfg[slot0.stageData:GetHeroDataByPos(slot8).using_skin].picture_id)
			end

			if slot4[slot8] and slot0:NeedAddExp() then
				slot0.heroAddExp[slot8].text = ""

				slot0:RemoveTweenAt(slot8)

				slot0.heroExp_[slot8] = slot4[slot8].newLv + slot4[slot8].newPersent
				slot0.tweens_[slot8] = LeanTween.value(slot0.heroProgress_[slot8].gameObject, slot4[slot8].oldLv + slot4[slot8].oldPersent, slot4[slot8].newLv + slot4[slot8].newPersent, 2):setEase(LeanTweenType.easeOutCubic):setOnUpdate(LuaHelper.FloatAction(function (slot0)
				...
				slot0.heroAddExp[slot8].text = ""
				slot0.heroLv_[slot8].text = slot9.level
				slot0.heroProgress_[slot8].value = 1
			end
		end
	end
end
...
function slot0.RefreshMyExpS(slot0)
	...
	slot13 = -1
	slot0.tween_ = LeanTween.value(slot0.progress_.gameObject, slot2 + slot11, slot9 + slot12, 2):setEase(LeanTweenType.easeOutCubic):setOnUpdate(LuaHelper.FloatAction(function (slot0)
...
function slot0.RefreshMissionS(slot0)
	...
	for slot6 = 1, 3 do
		if slot1 ~= nil and slot1[slot6] ~= nil then
			...
			slot0.missionText_[slot6].text = GetI18NText(BattleInstance.GetStarMissionText(slot1[slot6].id, slot1[slot6].xData, slot1[slot6].yData))
			slot8 = nil
...
function slot0.RefreshBottomBtn(slot0)
	...
	if slot2 then
		slot0.onceMoreLab_.text = GetI18NText(slot2)
	else
...
function slot0.CheckBattleError(slot0)
