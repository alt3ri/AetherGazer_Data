...
function slot1.SetLevelTitle(slot0)
	slot0.stareText_.text = GetI18NText(BattleSequentialBattleStageCfg[slot0.stageId].name)
...
function slot1.RefreshLeftPanel(slot0)
	...
	if SequentialBattleChapterCfg[slot1].boss_list[slot2] ~= 0 then
		...
		slot0.portraitImage_.sprite = getSpriteWithoutAtlas(string.format("TextureConfig/MardukUI/boss/icon/%s", slot3.boss_list[slot2]))
	else
...
function slot1.OnClickNextStage(slot0)
	...
	BattleController.GetInstance():LaunchBattle(BattleStageFactory.Produce(BattleConst.STAGE_TYPE_NEW.SEQUENTIAL_BATTLE, SequentialBattleChapterCfg[slot1].stage_id[slot2], slot1, ReserveParams.New(ReserveConst.RESERVE_TYPE.SEQUENTIAL_BATTLE, slot1, slot2, {
		stageType = BattleConst.STAGE_TYPE_NEW.SEQUENTIAL_BATTLE,
		stageID = SequentialBattleChapterCfg[slot1].stage_id[slot2],
		activityID = slot1
	})))
end
