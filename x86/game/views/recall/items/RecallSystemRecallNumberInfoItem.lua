...
function slot0.UpdateView(slot0)
	slot0.recallNameTxt_.text = slot0.data.base_info.nick
	slot0.recallUIDTxt_.text = slot0.data.user_id
	slot0.recallHeadshotImg_.sprite = ItemTools.getItemSprite(slot0.data.base_info.icon)

	slot0.recallHeadshotImg_:SetNativeSize()

	slot0.serverName.text = string.format(GetTips("RECALLED_SERVER_NAME"), slot0.data.server_name)
end
