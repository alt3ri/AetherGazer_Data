return {
	...
	CalWeaponAttribute = function (slot0, slot1, slot2)
		slot3 = {
			[slot7.attribute[1]] = slot7.attribute[2]
		}
		...
		slot7 = HeroCfg[slot0].weapon_break_attribute[slot1.breakthrough + 1]
		slot3[slot7[1]] = slot7[2] + (slot3[slot7[1]] or 0)
	...
	GetWeaponModuleDes = function (slot0)
		...
		return string.format(GetTips("WEAPON_MODULE_SKILL_TIP_TEMPLATE_4"), ItemTools.getItemName(slot1), GetCfgDescription(WeaponEffectCfg[WeaponServantCfg[slot1].effect[1]].strengthen_description[2], 1))
	end
}
