return {
	...
	SortBySelectId = function (slot0, slot1)
		...
		if slot1 ~= 0 and uv0.lastSort then
			if table.keyof(slot2, slot1) and table.keyof(uv0.lastSort, slot1) ~= slot5 then
	...
	CheckIsPurchaseTemplate = function (slot0)
		if tostring(slot0) == tostring(MailConst.MAIL_ID_CONST.SKIN_DLC_EXPIRE) or tostring(slot0) == tostring(MailConst.MAIL_ID_CONST.SKIN_EXPIRE) then
			return true
		end
