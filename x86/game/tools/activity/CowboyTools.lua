...
return {
	...
	DrawCowboy = function (slot0)
		...
		slot5 = -slot0.cowboyData_.cowboyPosY
		slot1.transform.localPosition = Vector2(slot0.cowboyData_.cowboyPosX, slot5)
		slot0.cowboyGo_ = slot1
		slot0.aniController_ = slot0.cowboyGo_:GetComponent("Animator")
	...
	EndGame = function (slot0)
		...
		if ActivityTools.ActivityOpenCheckByMessageBox(slot0.mainActivityID_, function ()
			...
			CowboyAction:SendActivityInfo(slot0.mainActivityID_, slot0.activityID_, slot0.score_, function ()
	...
	UpdateSelf = function (slot0)
		if slot0.onTimeChange_ then
			slot0.onTimeChange_(tostring(math.ceil(slot0.curTime_ > 0 and slot0.curTime_ or 0)))
		end
	...
	CheckGet = function (slot0)
		...
		for slot8, slot9 in ipairs(slot0.show_) do
			slot4 = slot9:GetHitRect()

			if not slot9:GetIsHitOrTaunt() then
				...
				if slot0.cowboyState_ == "attack" and (slot11 <= slot10 and slot10 - slot11 < slot0.hitLeft_ + slot4.hitRight or slot10 < slot11 and slot11 - slot10 < slot0.hitRight_ + slot4.hitLeft) then
					slot3 = math.max(slot9:GetData().index, -1)
				end

				if slot10 < slot11 and slot11 - slot10 > slot0.hitRight_ + slot4.hitLeft and not slot9:GetIsPass() then
					slot9:SetIsPass()
					slot9:SetState("runAway")
				end
			end
		end
		...
		if slot3 >= 0 and not slot0.isHitted_ then
			for slot8, slot9 in ipairs(slot0.show_) do
				if slot3 == slot9:GetData().index then
					slot0.isHitted_ = true

					slot9:SetState("fall")
					slot9:SetIsHitOrTaunt()
	...
	CheckNewCow = function (slot0)
		while slot0.cowIndex_ <= #slot0.cows_ and slot0.cows_[slot0.cowIndex_].showTime <= slot0.totalTime_ - slot0.curTime_ do
			if slot0.unshow_[slot0.cows_[slot0.cowIndex_].cowId] and #slot0.unshow_[slot1.cowId] > 0 then
				...
				slot3.transform.localPosition = Vector2(-uv0 / 2 - slot4.size.x / 2, slot0.cowboyData_.deltaY)
	...
	GameOver = function (slot0)
		...
		for slot4, slot5 in ipairs(slot0.show_ or {}) do
			slot5:OnExit()
		end

		for slot4, slot5 in ipairs(slot0.cowIDList_ or {}) do
			if slot0.unshow_[slot5] then
				for slot9, slot10 in ipairs(slot0.unshow_[slot5]) do
					slot10:OnExit()
				end
