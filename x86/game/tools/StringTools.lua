...
function WordVerifyBySDK(slot0, slot1, slot2)
	...
	if not OperationData:IsOperationOpen(OperationConst.MANUAL_WORD_VERIFY) then
		slot4 = wordVerify(string.gsub(slot0, " ", ""))
	end
	...
	if GameToSDK.CURRENT_SDK_ID == SDK_PLATFORM.DEV then
		slot7 = slot6 and ChatConst.VERIFY_STATUS.PASS or ChatConst.VERIFY_STATUS.LOCAL_NO_PASS

		SendJudgeResult(slot2, slot0, slot7, "")
		slot1(slot6, slot7)
	...
	if not slot6 then
		SendJudgeResult(slot2, slot0, ChatConst.VERIFY_STATUS.LOCAL_NO_PASS, "")
		slot1(slot6, ChatConst.VERIFY_STATUS.LOCAL_NO_PASS)

		return
	end

	slot1(true, ChatConst.VERIFY_STATUS.PASS)
end
...
function wordVerMatch(slot0, slot1, slot2, slot3, slot4, slot5, slot6, slot7)
	...
	for slot13, slot14 in ipairs(slot1) do
		if slot9 then
			table.deepMerge(slot9, slot14[slot0[slot3]] or {})
		else
...
function isContainChineseNumEnglish(slot0)
	...
	while slot1 <= #slot0 do
		if string.byte(slot0, slot1) >= 48 and slot2 <= 57 or slot2 >= 65 and slot2 <= 90 or slot2 >= 97 and slot2 <= 122 then
			...
			if 14989440 > string.byte(slot0, slot1 + 2) + string.byte(slot0, slot1 + 1) * 256 + slot2 * 65536 or 15318693 < slot7 then
				return false
			else
				slot1 = slot1 + 3
			end
...
function textLimit(slot0, slot1)
	...
	while slot2 <= #slot0 do
		...
		for slot10 = 0, getByteLength(string.byte(slot0, slot2)) - 1 do
			slot6 = 0 * 256 + string.byte(slot0, slot2 + slot10)
		end

		if slot1 < slot3 + (isNumOrEn(slot6) and 1 or isChinese(slot6) and 2 or isJanpanese(slot6) and 2 or isKorean(slot6) and 2 or isSymbolForTwoWord(slot6) and 2 or isSymbolForOneWord(slot6) and 1 or 1) then
...
function isSymbolForOneWord(slot0)
	return 32 <= slot0 and slot0 <= 127 or 49824 <= slot0 and slot0 <= 50111 or slot0 >= 48 and slot0 <= 57 or slot0 >= 65 and slot0 <= 90 or slot0 >= 97 and slot0 <= 122
end

function isSymbolForTwoWord(slot0)
	return 14909568 <= slot0 and slot0 <= 14909631 or 15711360 <= slot0 and slot0 <= 15712175 or 14844032 <= slot0 and slot0 <= 14844335 or slot0 >= 48 and slot0 <= 57 or slot0 >= 65 and slot0 <= 90 or slot0 >= 97 and slot0 <= 122
end
...
function nameRule(slot0, slot1)
	if AreaDifferenceCfg[GameToSDK.CURRENT_SERVER] then
		...
		while slot4 <= #slot0 do
			for slot10 = 0, getByteLength(string.byte(slot0, slot4)) - 1 do
				slot6 = 0 * 256 + string.byte(slot0, slot4 + slot10)
			end

			slot7 = false

			if isChinese(slot6) then
				slot7 = table.indexof(slot3.language_sent, 0) ~= false
...
function wordVerify(slot0, slot1)
	...
	while slot6 <= #wordSplit(slot0) do
		slot9, slot6, slot11 = wordVerMatch(slot3, slot4, slot1, slot6, "", false, slot6, "")
...
function utf8sub(slot0, slot1, slot2)
	...
	return slot0:sub(slot3, slot4 - 1), slot2
end
...
function formatNumber(slot0)
	...
	if slot0 < 10000000 then
		return math.floor(slot0 / 1000) .. "K"
	elseif slot0 < 10000000000.0 then
		return math.floor(slot0 / 1000000) .. "M"
	elseif slot0 < 10000000000000.0 then
		return math.floor(slot0 / 1000000000) .. "B"
	elseif slot0 < 1e+16 then
		return math.floor(slot0 / 1000000000000.0) .. "T"
	else
		return slot0
	end
end
