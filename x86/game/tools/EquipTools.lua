...
return {
	...
	GetFreeEquipByEquipId = function (slot0, slot1)
		...
		for slot10, slot11 in pairs(EquipData:GetEquipList()) do
	...
	GetHaveSkillList = function (slot0)
		...
		if slot0 then
			for slot7, slot8 in ipairs(EquipSkillCfg.get_id_list_by_skill_type[EquipConst.EQUIP_ATTRIBUTE_TYPE.ENCHANT]) do
	...
	CalHeroVirtualEquipAttribute = function (slot0, slot1, slot2)
		...
		for slot7, slot8 in pairs(slot0) do
			if slot8.prefab_id ~= 0 then
				...
				for slot12, slot13 in pairs(uv0.CountEquipAttribute(slot8, slot12, slot13)) do
	...
	QuickSelectList = function (slot0, slot1)
		...
		for slot7, slot8 in ipairs(ItemCfg.get_id_list_by_type[ItemConst.ITEM_TYPE.MATERIAL]) do
	...
	GetEffectDescOld = function (slot0)
		...
		for slot7 in string.gmatch(slot1.desc, "@{.-}") do
