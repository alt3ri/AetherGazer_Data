...
return {
	typeToFunction = {
		...
		purchase_swigs = function (slot0)
			...
			if slot0.buyFatigueNum == 5 then
				SDKTools.SetParamsAttribute("purchase_5swigs", uv0)
			elseif slot0.buyFatigueNum == 10 then
				SDKTools.SetParamsAttribute("purchase_10swigs", uv0)
			elseif slot0.buyFatigueNum == 20 then
				SDKTools.SetParamsAttribute("purchase_20swigs", uv0)
			elseif slot0.buyFatigueNum == 30 then
				SDKTools.SetParamsAttribute("purchase_30swigs", uv0)
			end
		end,
		...
		charge_Total = function (slot0)
			if slot0.total >= 50 and slot0.total < 100 then
				SDKTools.SetBaseAttribute("charge_junior")
			elseif slot0.total >= 100 and slot0.total < 500 then
				SDKTools.SetBaseAttribute("charge_medium")
			elseif slot0.total >= 500 then
				SDKTools.SetBaseAttribute("charge_senior")
			end
		end,
		purchase_complete_GP_ONCE = function ()
			SDKTools.SetBaseAttribute("purchase_complete_GP_ONCE", uv0)
		end
	}
}
