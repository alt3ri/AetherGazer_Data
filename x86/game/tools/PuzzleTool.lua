return {
	...
	RefreshPuzzleBg = function (slot0, slot1)
		if getSpriteWithoutAtlas("TextureConfig/VolumeIIIUI/" .. ActivityPuzzleCfg[slot1].album_id) ~= nil then
	...
	ScaleBg = function (slot0, slot1)
		...
		slot0.bgScaleLeanTween_ = LeanTween.scale(slot0.bgTrans_, slot2, 0.466):setEase(LeanTweenType.easeOutCubic):setOnComplete(LuaHelper.VoidAction(function ()
	...
	RecoverBg = function (slot0)
		if slot0.bgOriginScale_ ~= nil then
			slot0.bgScaleLeanTween_ = LeanTween.scale(slot0.bgTrans_, slot0.bgOriginScale_, 0.466):setEase(LeanTweenType.easeOutCubic):setOnComplete(LuaHelper.VoidAction(function ()
				if uv0.bgScaleLeanTween_ then
					uv0.bgScaleLeanTween_:setOnComplete(nil)
