return {
	Play60306001 = function (slot0, slot1)
		slot1.time_ = 0
		slot1.frameCnt_ = 0
		slot1.state_ = "playing"
		slot1.curTalkId_ = 60306001
		slot1.duration_ = 5
		...
		function slot1.onSingleLineUpdate_(slot0)
			...
			if slot3 <= uv0.time_ and uv0.time_ < slot3 + slot4 then
				slot2.localPosition = Vector3.Lerp(uv0.var_.moveOldPos2440001, Vector3.New(0, -180, -800), (uv0.time_ - slot3) / slot4)
			end

			if uv0.time_ >= slot3 + slot4 and uv0.time_ < slot3 + slot4 + slot0 then
				slot2.localPosition = Vector3.New(0, -180, -800)
			end
			...
			if 0 < uv0.time_ and uv0.time_ <= slot5 + slot0 then
				...
				uv0.leftNameTxt_.text = uv0:FormatText(StoryNameCfg[125].name)
				...
				slot10 = uv0:FormatText(uv0:GetWordFromCfg(60306001).content)
