return {
	Play100 = function (slot0, slot1)
		...
		function slot1.onSingleLineUpdate_(slot0)
			...
			if 2 < uv0.time_ and uv0.time_ <= slot3 + slot0 and uv0.var_.characterEffect1148ui_story == nil then
			...
			if 2 < uv0.time_ and uv0.time_ <= slot22 + slot0 then
				uv0.locationGo_:SetActive(true)
				uv0.locationAnimator_:Play("btn_location_cx")

				uv0.locationTitleText_.text = uv0:FormatText(uv0:GetWordFromCfg(100).content)

				LuaForUtil.ClearLinePrefixSymbol(uv0.locationTitleText_)

				uv0.locationTitleText_:GetComponent("Text").color = Color(1, 1, 1)
				uv0.locationTitleText_.font = Asset.Load("Fonts/SourceHanSans")
				uv0.locationNameText_.text = uv0:FormatText(uv0:GetWordFromCfg(100).content)

				LuaForUtil.ClearLinePrefixSymbol(uv0.locationNamseText_)

				uv0.locationNameText_:GetComponent("Text").color = Color(1, 1, 1)
				uv0.locationNameText_.font = Asset.Load("Fonts/SourceHanSans")
			end

			if uv0.time_ >= slot22 + slot21 and uv0.time_ < slot22 + slot21 + slot0 then
				uv0.locationAnimator_:Play("btn_location_xs")
			end
			...
			if 2 < uv0.time_ and uv0.time_ <= slot23 + slot0 then
				...
				slot27 = uv0:FormatText(uv0:GetWordFromCfg(100).content)
