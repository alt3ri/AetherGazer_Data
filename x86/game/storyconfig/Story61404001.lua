return {
	Play61404001 = function (slot0, slot1)
		slot1.time_ = 0
		slot1.frameCnt_ = 0
		slot1.state_ = "playing"
		slot1.curTalkId_ = 61404001
		...
		function slot1.onSingleLineUpdate_(slot0)
			if uv0.actors_["2530001"] == nil then
			...
			slot2 = uv0.actors_["2530001"].transform

			if 0.333333333333333 < uv0.time_ and uv0.time_ <= slot3 + slot0 then
				uv0.var_.moveOldPos2530001 = slot2.localPosition
			...
			if slot3 <= uv0.time_ and uv0.time_ < slot3 + slot4 then
				slot2.localPosition = Vector3.Lerp(uv0.var_.moveOldPos2530001, Vector3.New(0, -105, -200), (uv0.time_ - slot3) / slot4)
			end

			if uv0.time_ >= slot3 + slot4 and uv0.time_ < slot3 + slot4 + slot0 then
				slot2.localPosition = Vector3.New(0, -105, -200)
			end

			if 0.333333333333333 < uv0.time_ and uv0.time_ <= slot6 + slot0 and uv0.actors_["2530001"]:GetComponent("Image") then
				uv0.var_.highlightMatValue2530001 = slot7
			end

			slot7 = 0.2

			if slot6 <= uv0.time_ and uv0.time_ < slot6 + slot7 then
				if uv0.var_.highlightMatValue2530001 then
					slot9 = Mathf.Lerp(0.5, 1, (uv0.time_ - slot6) / slot7)
					slot10 = uv0.var_.highlightMatValue2530001
					slot11 = slot10.color
					slot11.r = slot9
					slot11.g = slot9
					slot11.b = slot9
			...
			if uv0.time_ >= slot6 + slot7 and uv0.time_ < slot6 + slot7 + slot0 and uv0.var_.highlightMatValue2530001 then
				slot8 = 1

				slot5.transform:SetSiblingIndex(1)

				slot9 = uv0.var_.highlightMatValue2530001
				slot10 = slot9.color
				slot10.r = slot8
				slot10.g = slot8
				slot10.b = slot8
			...
			if 0.333333333333333 < uv0.time_ and uv0.time_ <= slot8 + slot0 then
				...
				uv0.leftNameTxt_.text = uv0:FormatText(StoryNameCfg[165].name)
				...
				slot13 = uv0:FormatText(uv0:GetWordFromCfg(61404001).content)
				uv0.text_.text = slot13

				LuaForUtil.ClearLinePrefixSymbol(uv0.text_)

				if (14 <= 0 and slot9 or slot9 * utf8.len(slot13) / slot14) > 0 and slot9 < slot16 then
