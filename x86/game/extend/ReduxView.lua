...
function slot0.AdaptLeft(slot0)
	if slot0.leftTrs_ == nil then
		if slot0.leftGo_ == nil then
			slot0.leftGo_ = slot0:FindGo("panel/left") or slot0:FindGo("Canvas/panel/left")
		end

		slot0.needAdaptLeft_ = false

		if slot0.leftGo_ == nil then
			slot0.leftGo_ = slot0:FindGo("panel/left_adapt") or slot0:FindGo("Canvas/panel/left_adapt")
...
function slot0.AdaptRight(slot0)
	if slot0.rightTrs_ == nil then
		if slot0.rightGo_ == nil then
			slot0.rightGo_ = slot0:FindGo("panel/right") or slot0:FindGo("Canvas/panel/right")
		end

		slot0.needAdaptRight_ = false

		if slot0.rightGo_ == nil then
			slot0.rightGo_ = slot0:FindGo("panel/right_adapt") or slot0:FindGo("Canvas/panel/right_adapt")
...
function slot0.AdaptMiddle(slot0)
	if slot0.middleTrs_ == nil then
		if slot0.middleGo_ == nil then
			slot0.middleGo_ = slot0:FindGo("panel/middle_adapt") or slot0:FindGo("Canvas/panel/middle_adapt")
		end
...
function slot0.AdaptChild(slot0)
	if slot0.containerTrs_ == nil then
		if slot0.containerGo_ == nil then
			slot0.containerGo_ = slot0:FindGo("panel/pageContainer") or slot0:FindGo("Canvas/panel/pageContainer")
		end
...
function slot0.CameraEnter(slot0, slot1)
	if slot1 and slot0:UIParent() ~= manager.ui.uiPop.transform then
...
function slot0.SetRouteName(slot0, slot1)
	slot0.routeName_ = slot1
end
...
function slot0.RealCheckWeakGuide(slot0)
	if manager.guide:IsPlaying() then
		return
	end

	slot1, slot2 = GuideTool.CheckWeakGuide(slot0.routeName_)

	if slot1 then
		slot4 = nil
		slot5 = GuideTool.GetGuideComponentByRoute(slot0, slot2.mask_component)

		function slot6()
			if uv0 then
				manager.guide:ShowWeakView(uv1, uv0, uv2)
			else
				NewPlayerGuideAction.FinishWeakGuide(uv1.id)
				GuideTool.Log("未找到弱引导组件，弱引导id:" .. uv1.id)
			end
		end

		if string.sub(slot2.guide_component[1], 1, 2) == "@@" then
			slot7 = nil

			FrameTimer.New(function ()
				uv0 = uv1:GetSpecialCom(uv2.guide_component)

				uv3()
				uv4:Stop()
			end, 10, 1):Start()
		else
			slot4 = GuideTool.GetGuideComponentByRoute(slot0, slot2.guide_component)
...
function slot0.HideWeakGuide(slot0)
