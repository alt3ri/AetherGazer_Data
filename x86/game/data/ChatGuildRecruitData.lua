...
function slot0.SortMsg(slot0, slot1)
	table.sort(slot0.unsortRecruitData_, function (slot0, slot1)
	...
	for slot5, slot6 in ipairs(slot0.unsortRecruitData_) do
		...
		table.insert(slot0.guildRecruitData_, slot6)
...
function slot0.InitCacheContent(slot0)
	slot2 = slot0.guildRecruitData_ or {}
	slot0.guildRecruitTempData_ = {}
...
function slot0.AddChatMsg(slot0, slot1, slot2)
	if not slot2 then
		for slot6, slot7 in ipairs(slot0.guildRecruitTempData_) do
			if slot7.guildID and slot7.guildID == slot1.guildID then
...
function slot0.RemoveChatRecord(slot0, slot1)
	for slot5, slot6 in ipairs(slot0.guildRecruitData_) do
		if slot6.contentType == ChatConst.CHAT_CONTENT_TYPE.GUILD_RECRUIT and slot6.msgID == slot1 then
			if slot0.guildRecruitData_[slot5 + 1] == nil and slot0.guildRecruitData_[slot5 - 1] and slot0.guildRecruitData_[slot5 - 1].contentType == ChatConst.CHAT_CONTENT_TYPE.TIMESTAMP then
	...
	for slot5, slot6 in ipairs(slot0.guildRecruitTempData_) do
		if slot6.contentType == ChatConst.CHAT_CONTENT_TYPE.GUILD_RECRUIT and slot6.msgID == slot1 then
			if slot0.guildRecruitTempData_[slot5 + 1] and slot0.guildRecruitTempData_[slot5 - 1] and slot0.guildRecruitTempData_[slot5 - 1].contentType == ChatConst.CHAT_CONTENT_TYPE.TIMESTAMP then
