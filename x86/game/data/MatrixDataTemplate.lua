...
MatrixTools = {
	GetMatrixItemSprite = function (slot0)
		if MatrixItemCfg[slot0].matrix_item_type == MatrixConst.ITEM_TYPE.TREASURE or slot1.matrix_item_type == MatrixConst.ITEM_TYPE.EFFECT then
			return getSpriteWithoutAtlas("TextureConfig/MatrixItem/MatrixArtifact/" .. slot1.icon)
		...
		elseif slot1.matrix_item_type == MatrixConst.ITEM_TYPE.EQUIP then
			return getSpriteWithoutAtlas(SpritePathCfg.EquipIcon_s.path .. slot1.icon)
	...
	GetMatrixItemDes = function (slot0, slot1, slot2)
		if MatrixItemCfg[slot0].matrix_item_type == MatrixConst.ITEM_TYPE.ASTROLABE then
			if slot1 then
				return GetCfgDescription(AstrolabeEffectCfg[HeroAstrolabeCfg.get_id_list_by_hero_astrolabe_suit_id[slot3.params[1]][math.min(3, slot1)]].desc[1], 1)
			end
...
function MatrixTools.GetGiftListByGroup(slot0)
	...
	for slot9 = 1, slot5 do
		...
		for slot16 = slot10, (slot9 % 2 ~= 0 or math.min(#slot3, math.max(1, slot9 / 2 * 5 - 1) + 1)) and math.min(#slot3, math.max(1, math.ceil(slot9 / 2) * 5 - 4) + 2) do
...
function MatrixTools.UpdateSortScoreList()
	...
	table.sort(MatrixData:GetPointRewardList(), function (slot0, slot1)
		slot2 = slot0.need_level <= uv0

		if slot1.is_got_reward == slot0.is_got_reward then
			if slot2 == (slot1.need_level <= uv0) then
...
function MatrixHeroTemplate.InitData(slot0, slot1)
	...
	table.sort(slot0.astrolabe_list, function (slot0, slot1)
		return slot0 < slot1
	end)
	...
	table.sort(slot0.equip_list, function (slot0, slot1)
		return slot0 < slot1
	end)
...
function MatrixOwnerHeroTemplate.InitData(slot0, slot1)
	...
	if slot1.entry_snapshot.weapon.servant then
		-- Nothing
	else
		slot3.servantInfo = {}
	end
