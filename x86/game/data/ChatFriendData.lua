...
function slot0.AddChat(slot0, slot1)
	slot3 = USER_ID == slot1.chat_base_info.id and slot1.receive_uid or slot1.chat_base_info.id
	slot0.chatRecordList_[slot3] = slot0.chatRecordList_[slot3] or {}

	if slot0:IsOldMsg(slot3, slot1.chat_base_info.msg_id) then
		return
	end
...
function slot0.IsOldMsg(slot0, slot1, slot2)
	for slot7 = 0, 9 do
		if slot0.chatRecordList_[slot1][(slot0.chatRecordList_[slot1] and #slot0.chatRecordList_[slot1] or 0) - slot7] and slot8.msgID == slot2 then
			return true
		end
	end
...
function slot0.ParseChat(slot0, slot1)
	return {
		recall = false,
		msgID = slot1.chat_base_info.msg_id,
		friendID = USER_ID == slot1.chat_base_info.id and slot1.receive_uid or slot1.chat_base_info.id,
		senderID = slot1.chat_base_info.id,
		timestamp = slot1.chat_base_info.timestamp,
		contentType = slot1.chat_base_info.type,
		content = slot1.chat_base_info.content
	}
end
...
function slot0.UpdateTimestamp(slot0, slot1)
	...
	if slot0.friendList_[slot3] == nil then
		...
		slot0.friendList_[slot3].timestamp = slot1.chat_base_info.timestamp
	end
end
...
function slot0.UpdateCacheFriendContent(slot0, slot1)
	if slot0.historyTipsFlag_[USER_ID == slot1.chat_base_info.id and slot1.receive_uid or slot1.chat_base_info.id] ~= true then
		if slot1.chat_base_info.id ~= slot2 then
...
function slot0.GetUnreadMsgCnt(slot0, slot1)
	if slot0.chatRecordList_[slot1] then
		...
		for slot6 = #slot0.chatRecordList_[slot1], 1, -1 do
			...
			if slot0.chatRecordList_[slot1][slot6].recall == false and slot0.chatRecordList_[slot1][slot6].senderID == slot1 then
				slot2 = slot2 + 1
			end
		end
