...
function slot0.Init(slot0)
	...
	for slot4, slot5 in ipairs(ChipCfg.get_id_list_by_type_id[ChipConst.TYPE_MANAGER]) do
	...
	for slot4, slot5 in ipairs(ChipCfg.get_id_list_by_type_id[ChipConst.TYPE_CHIP]) do
	...
	for slot4, slot5 in pairs(ChipCfg.get_id_list_by_spec_char) do
		slot0.enabledheroChipList_[slot4] = {
			0,
			0,
			0,
			0
	...
	for slot4, slot5 in ipairs(ChipCfg.get_id_list_by_type_id[ChipConst.TYPE_HERO_CHIP]) do
...
function slot0.InitData(slot0, slot1)
	...
	slot0.enabledChipManagerID_ = slot1.chip or 0

	for slot5, slot6 in ipairs(slot1.unlock_kernel_chip) do
		table.insert(slot0.unlockChipManagerIDList_, slot6)
	end
	...
	for slot5, slot6 in ipairs(slot1.hero_chip_state) do
		for slot10, slot11 in ipairs(slot6.secondary) do
			if slot11 > 0 then
				slot0.enabledheroChipList_[slot6.hero_id][ChipCfg[slot11].role_type_id] = slot11
			end
		end
	end
end
...
function slot0.GetIsUnlockHeroChip(slot0, slot1)
	for slot5, slot6 in pairs(slot0.unlockHeroChip_) do
		if slot6 == slot1 then
			return true
		end
	end
...
function slot0.GetHeroChipIsEnableByChipId(slot0, slot1)
	return slot0.enabledheroChipList_[ChipCfg[slot1].spec_char][ChipCfg[slot1].role_type_id] == slot1, slot4
end
...
function slot0.EnableHeroChip(slot0, slot1, slot2, slot3)
	slot0.enabledheroChipList_[slot1][slot3] = slot2
end
	...
	if slot2 then
		for slot8, slot9 in ipairs(slot2) do
			if slot0.enabledheroChipList_[slot9] then
				for slot13, slot14 in pairs(slot0.enabledheroChipList_[slot9]) do
					if slot14 > 0 then
