...
function slot0.InitArchive(slot0, slot1)
	...
	for slot5, slot6 in ipairs(cleanProtoTable(slot1)) do
		slot11 = slot6.exp
		slot10 = {}
		uv0[slot6.archive_id] = {
			archive_id = slot6.archive_id,
			exp = slot6.exp,
			lv = LvTools.LoveExpToLevel(slot11),
			gift_list = {},
			text_list = {},
			video_list = {},
		...
		for slot10, slot11 in ipairs(cleanProtoTable(slot6.gift_list)) do
			uv0[slot6.archive_id].gift_list[slot11.id] = slot11.num
		end

		for slot10, slot11 in ipairs(cleanProtoTable(slot6.text_list)) do
			uv0[slot6.archive_id].text_list[slot11] = true
		end

		for slot10, slot11 in ipairs(cleanProtoTable(slot6.video_list)) do
			uv0[slot6.archive_id].video_list[slot11] = true
		end

		for slot10, slot11 in ipairs(cleanProtoTable(slot6.hero_story_list)) do
			uv0[slot6.archive_id].hero_story_list[slot11.hero_id] = slot11
		end

		for slot10, slot11 in ipairs(cleanProtoTable(slot6.super_heart_link_list)) do
