...
function slot0.InitRedPoint(slot0)
	...
	for slot6, slot7 in pairs(TowerGameCfg.all) do
		slot11 = "%s_%s"
		slot12 = RedPointConst.MARDUK_TOWERGAME_NOT_FIGHT

		table.insert(slot1, string.format(slot11, slot12, slot7))
...
function slot0.GetAllRewardNum(slot0)
	for slot5, slot6 in pairs(TowerGameCfg.all) do
		slot1 = 0 + #ActivityPointRewardCfg.get_id_list_by_activity_id[slot6]
	end
