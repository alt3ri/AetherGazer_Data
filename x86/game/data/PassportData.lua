...
function slot0.GetBonusSuccess(slot0, slot1, slot2)
	...
	for slot6, slot7 in ipairs(slot2) do
		if not uv0.receiveInfo[slot7.id] then
			uv0.receiveInfo[slot7.id] = {}
		end

		table.insert(uv0.receiveInfo[slot7.id], {
			id = slot7.id,
			isPay = slot7.is_pay
		})
	end
...
function slot0.HaveGetBonus(slot0, slot1)
	if uv0.receiveInfo[slot1] then
		if uv0.payLevel == 0 then
			...
			return uv0.receiveInfo[slot1] ~= nil and #uv0.receiveInfo[slot1] == 2
		end
	end
...
function slot0.MarkAllBonusGet(slot0, slot1)
	if uv0.id == 0 then
		return
	end
...
function slot0.GetLevel(slot0)
	for slot5 = slot0:GetMaxLevel(), 1, -1 do
		if GameLevelSetting[slot5].battlepass_lv_exp_sum <= ItemTools.getItemNum(CurrencyConst.CURRENCY_TYPE_BATTLEPASS_EXP) then
			return slot5
		end
	end
...
function slot0.GetCurrentExp(slot0)
	slot1 = ItemTools.getItemNum(CurrencyConst.CURRENCY_TYPE_BATTLEPASS_EXP)
...
function slot0.GetCurrentStatus(slot0)
	if not slot0:IsOpen() then
		return 0, slot0:GetLevel() == slot0:GetMaxLevel() and 0 or slot1 + 1
	end
...
function slot0.GetRewardStatus(slot0, slot1, slot2)
	...
	slot4 = ItemTools.getItemNum(CurrencyConst.CURRENCY_TYPE_BATTLEPASS_EXP)

	if slot0:HaveGetBonus(slot2) then
		return (slot0:GetPayLevel() > 0 and "pay" or "free") .. "HaveGet"
