...
function slot0.UpdateDailyTask(slot0, slot1)
	if slot1.daily_pt_task_got_flag == true then
		slot0.dailyTaskStatus_[slot1.activity_id] = true
	else
		slot0.dailyTaskStatus_[slot2] = nil
	end
...
function slot0.SetReceivedDailyTask(slot0, slot1)
	...
	slot0:RefreshAccumulateRedPoint(slot1)
end
...
function slot0.RefreshDailyRedPoint(slot0, slot1)
	...
	if 100 <= ActivityPtData:GetCurrentActivityPt(ActivityPtConst.TASK_DAILY_ACTIVITY_PT) and CultivateHeroData:GetDailyTaskStatus()[slot1] ~= true then
		slot5 = 1
	end
...
function slot0.RefreshAccumulateRedPoint(slot0, slot1)
	...
	for slot8, slot9 in ipairs(ActivityPointRewardCfg.get_id_list_by_activity_id[slot1]) do
		if ActivityPointRewardCfg[slot9].need <= ItemTools.getItemNum(ActivityCultivateHeroCfg[slot1].coin_id) and (slot0.accumulateTaskInfoList_[slot1][slot9] and slot0.accumulateTaskInfoList_[slot1][slot9].isReceived) ~= true then
			slot4 = 1
...
function slot0.RefreshHeroTaskRedPoint(slot0, slot1)
	...
	for slot6, slot7 in ipairs(ActivityCultivateHeroCfg[slot1].group) do
		...
		for slot13, slot14 in ipairs(CultivateHeroTaskCfg.get_id_list_by_group_id[slot7]) do
			if slot0.heroTaskInfoList_[slot1][slot14].isCompleted == true and not slot0.heroTaskInfoList_[slot1][slot14].isReceived then
				slot9 = 1
...
function slot0.IsAllReceived(slot0, slot1)
	...
	if slot3 == true then
		...
		slot8 = RedPointConst.CULTIVATE_HERO
		slot9 = slot1

		manager.redPoint:setTip(string.format("%s_%s_Accumulate_Reward", slot8, slot9), 0)
