...
function slot0.InitData(slot0, slot1)
	...
	if uv0 > 0 then
		...
		for slot7, slot8 in pairs(slot3) do
			if uv1 < slot8 and slot8 <= slot2 and uv1 < slot2 then
...
function slot0.SetSelectDifficult(slot0, slot1, slot2)
	...
	saveData("BossChallenge", "difficult" .. slot1, slot2)
end

function slot0.GetSelectDifficult(slot0, slot1)
	if slot0.selectDifficult_[slot1] then
		return slot0.selectDifficult_[slot1]
	end

	if slot0.finishIndex_[slot1] < (getData("BossChallenge", "difficult" .. slot1) or slot0.finishIndex_[slot1]) then
		return slot0.finishIndex_[slot1]
	else
		return slot2
	end
...
function slot0.CalcRedPoint(slot0)
	for slot6, slot7 in ipairs(slot0:GetBossChallengeCfg().reward) do
		if slot7[1] <= StageTools.CalcBossChallengeStar() and not table.keyof(slot0.receiveStarList_, slot7[1]) then
