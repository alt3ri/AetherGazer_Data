...
function slot0.InitData(slot0, slot1)
	...
	for slot5, slot6 in ipairs(slot1.boss_list) do
		...
		for slot12, slot13 in ipairs(slot6.affix_index_list) do
			table.insert(slot8, slot13)
		end

		slot13 = BossConst.TYPE_AFFIX
		...
		for slot13, slot14 in ipairs(slot6.used_heroes) do
			table.insert(slot0.lockHero_[slot5], slot14)
		end
		...
		for slot13, slot14 in ipairs(slot6.last_heroes_cfg) do
			slot0.lastHeroTeam_[slot5][slot13] = slot14 or 0
		end
	end
...
function slot0.GetPointValue(slot0, slot1)
	...
	for slot8, slot9 in pairs(slot2.condition[BossConst.TYPE_STAGE]) do
		slot4 = BossChallengeAdvanceCfg[slot0.chooseModeID_].difficult_point[slot2.multipleIndex] + StageChallengeConditionPoolCfg[slot9].point
	end

	for slot8, slot9 in pairs(slot2.condition[BossConst.TYPE_AFFIX]) do
...
function slot0.ModifyAllReceiveReward(slot0)
	for slot6, slot7 in ipairs(BossChallengeAdvanceCfg[slot0.chooseModeID_].reward) do
		if slot7[1] <= slot0:GetTotalPoint() and not table.keyof(slot0.receiveRewardList_, slot7[1]) then
...
function slot0.SortRewardList(slot0)
	...
	for slot8, slot9 in ipairs(BossChallengeAdvanceCfg[slot0.chooseModeID_].reward) do
