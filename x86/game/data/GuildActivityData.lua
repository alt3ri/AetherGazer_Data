...
function slot0.InitMainDataFromServer(slot0, slot1)
	...
	slot0:RefreshAffixRedPoint(ActivityConst.GUILD_ACTIVITY_START)
...
function slot0.CreateDefaultDataList(slot0)
	...
	for slot6, slot7 in ipairs({
		[table.indexof(ActivityClubMapCfg.all, slot6)] = slot9[#slot9]
	}) do
...
function slot0.OnGetCoin(slot0, slot1)
	...
	if GuildActivityLuaBridge.GetManager() then
		GuildActivityLuaBridge.GetManager():UpdateNodeData(slot0:CreateDefaultDataList())
	end
end
...
function slot0.GetHeroFatigue(slot0, slot1)
	...
	if uv0.hero_dic[slot1] then
		...
		return math.min(uv0.hero_dic[slot1].fatigue + math.floor((manager.time:GetServerTime() - slot3) / slot6) - math.floor((uv0.hero_dic[slot1].last_update_time - slot3) / slot6), GameSetting.activity_club_hero_fatigue_max.value[1])
	else
		return 0
	end
end
...
function slot0.GetTotalCoinToGet(slot0, slot1)
	for slot6, slot7 in pairs(uv0.grid_dic) do
		if slot7.occupied_timestamp > 0 then
			...
			slot2 = 0 + (math.floor((manager.time:GetServerTime() - slot12) / slot14) - math.floor((math.max(uv0.last_receive_time, slot7.occupied_timestamp) - slot12) / slot14)) * ActivityClubCfg[slot7.id].token_get / (3600 / slot14)
		end
	end
...
function slot0.GetSortedRateIDList(slot0, slot1)
	...
	for slot13, slot14 in ipairs(slot6) do
		if ActivityClubLevelSettingCfg[slot14].user_level <= ActivityClubLevelSettingCfg[slot0:GetCurRateID(slot1)].user_level and table.keyof(uv0.receive_list, ActivityClubLevelSettingCfg[slot14].user_level) == nil then
...
function slot0.RefreshRateRedPoint(slot0, slot1)
	...
	for slot11 = 2, #ActivityClubLevelSettingCfg.get_id_list_by_activity[slot1] do
		if ActivityClubLevelSettingCfg[slot2[slot11]].user_level <= ActivityClubLevelSettingCfg[slot0:GetCurRateID(slot1)].user_level and table.keyof(uv0.receive_list, slot7) == nil then
...
function slot0.RefreshAffixRedPoint(slot0, slot1)
	...
	for slot12, slot13 in ipairs(slot2) do
		...
		if ((TalentTreeCfg[slot13].cost[slot6] == nil or TalentTreeCfg[slot13].cost[slot6][2]) and -1) ~= -1 and slot7 <= (ItemTools.getItemNum(TalentTreeCfg[TalentTreeCfg.get_id_list_by_activity_id[slot1][1]].cost[1][1]) or 0) then
			manager.redPoint:setTip(string.format("%s_%d_%d_%d", RedPointConst.GUILD_ACTIVITY_AFFIX, slot1, TalentTreeCfg[slot13].race, slot13), 1)
		else
