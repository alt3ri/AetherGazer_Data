...
function slot0.RemoveCharacterAI(slot0, slot1)
	if nullable(slot0.characterList[slot1], "curNode") then
		slot2:Exit()
...
function slot0.RegisterEvent(slot0)
	slot5 = slot0.OnCmdAbort

	for slot5, slot6 in pairs({
		[DORM_DORMITORY_NEW_HERO] = handler(slot0, slot0.AddCharacterAI),
		[DORM_LOGIC_TICK] = handlerArg1(slot0, slot0.ProcessUpdate),
		[ON_DORM_CHARACTER_CMD_ABORTED] = handler(slot0, slot5)
	}) do
...
function slot0.ProcessUpdate(slot0)
	for slot4, slot5 in pairs(slot0.characterList) do
		slot0:ProcessEvent(DORM_LOGIC_TICK, slot4)
	end
end
...
function slot0.OnCmdAbort(slot0, slot1)
	if slot0.characterList[slot1] then
		slot0.ConsumeInteractCtx(slot1)
...
function slot0.ConsumeInteractCtx(slot0)
	...
	return uv0.GetEntityData(slot0).interactCtx
end
