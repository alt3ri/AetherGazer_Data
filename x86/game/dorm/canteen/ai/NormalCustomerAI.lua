...
function slot0.RegisterEvent(slot0)
	...
	slot1:Register(DORM_LOGIC_TICK, handler(slot0, slot0.Update))
...
function slot0.ListenPlayerInteractEvent(slot0, slot1, slot2, slot3)
	...
	for slot7, slot8 in ipairs(CanteenAIFunction:GetEntityData(slot7).peer) do
...
function slot0.AIChoice(slot0, slot1, slot2, ...)
	...
	elseif slot4 == uv0.NormalCustomerLeaveSeat then
		if slot2 == uv1 then
			if slot5.pay then
				if slot5.eat and slot0.NormalCustomerStateList[slot1].lastState ~= uv0.FvckOff and true then
...
function slot11(slot0)
	...
	if CanteenAIFunction:GetCurCanteenMode() == DormEnum.RestaurantMode.RestaurantManual then
		...
		for slot5, slot6 in pairs(CanteenFoodData:GetChooseFoodList()) do
			-- Nothing
		end
	end
	...
	for slot6, slot7 in pairs(slot0) do
		for slot12, slot13 in pairs(uv0(CanteenAIFunction:GetEntityData(slot7).cfgID)) do
...
function slot0.OrderFood(slot0)
	...
	if next(uv0(slot0)) then
		if CanteenAIFunction:GetCurCanteenMode() == DormEnum.RestaurantMode.RestaurantManual then
			...
			for slot6 = 1, math.min(DormConst.ORDER_FOOD_NUM, slot6) do
...
function slot0.CalculateFoodQuality(slot0, slot1, slot2, slot3)
	...
	for slot11, slot12 in ipairs(slot2) do
		...
		CanteenManualData:RecordFoodDelivery(CanteenAIFunction:GetFoodManualIndex(slot12), slot1, slot4.cfgID)
	end
	...
	if CanteenAIFunction:GetCurCanteenMode() == DormEnum.RestaurantMode.RestaurantManual then
		CanteenManualData:RecordFoodQuality(slot2, slot6)
		CanteenManualData:AddPopularityNum(slot6)
	end
