...
function slot0.ChooseByRestaurantOrders()
	...
	for slot5, slot6 in pairs(DormEnum.RestaurantKitchenWare) do
		...
		for slot11, slot12 in Dorm.storage:ForeachData(CanteenItemManager.ItemEIdNamespace(slot6), slot11) do
			if uv0(slot12) then
				slot1[slot6] = slot12
...
function slot0.PickFood(slot0)
	for slot4, slot5 in pairs(DormEnum.RestaurantKitchenWare) do
		...
		for slot10, slot11 in Dorm.storage:ForeachData(CanteenAIFunction:GetNameSpace(slot5), slot10) do
			if uv0(slot11) then
				return slot11
			end
		end
...
function slot0.MoveToTarget(slot0, slot1)
	...
	if slot2 == slot0.passtableEID then
		slot0:Move(slot1, slot0:ToTargetPos(slot1, slot2))
	else
