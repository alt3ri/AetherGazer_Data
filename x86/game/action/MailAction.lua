manager.net:Bind(30001, function (slot0)
	MailData.MailInit(slot0)
	uv0.UpdateMailUnread()
end)
manager.net:Bind(30013, function (slot0)
		...
		if isSuccess(slot0.result) then
			...
			if #slot0.attachment_list > 0 then
				...
				for slot6, slot7 in ipairs(slot0.attachment_list) do
					if ItemCfg[slot7.id].sub_type == 404 or slot8.sub_type == 1702 then
						slot9 = slot8.time
						...
						table.insert(slot2, {
							...
							slot7.num,
							0,
							ItemTools.GetItemExpiredTimeByID(slot7.id)
						})
					else
