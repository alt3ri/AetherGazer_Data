...
manager.net:Bind(33999, function (slot0)
	GuildData:UpdateBossDamage(slot0.boss_challenge_total_damage)
	uv0.UpdateGuildBossRedPoint()
end)
manager.net:Bind(27405, function (slot0)
	GuildData:SetShareTimestamp(slot0)
...
return {
	...
	RequiredGuildRecommendList = function (slot0)
		manager.net:SendWithLoadingNew(31006, {
			...
			GuildData:InitGuildRecommendList(slot0.recommend_club_list)
			uv0(slot0)
		end)
	end,
	...
	OnGetGuildBossRewardCallback = function (slot0, slot1)
		if isSuccess(slot0.result) then
			if slot1.type == 1 then
				...
				for slot5, slot6 in ipairs(ClubBossCfg[slot4.GetBossDifficulty(slot5)].personal_reward) do
					if slot6[1] == slot1.award_id then
						getReward(mergeReward(formatRewardCfgList(slot6[2])))
			...
			elseif slot1.type == 2 then
				...
				for slot5, slot6 in ipairs(ClubBossCfg[slot4.GetBossDifficulty(slot5)].reward) do
					if slot6[1] == slot1.award_id then
						getReward(mergeReward(formatRewardCfgList(slot6[2])))
	...
	SetAssistHero = function (slot0, slot1, slot2)
		slot4 = clone(GuildData:GetCurrentBossAssistantHeroId() or {})

		if slot2 then
			if GuildData:CheckCurrentHeroIDIsAssistant(slot1) then
			...
			if not GuildData:CheckCurrentHeroIDIsAssistant(slot1) then
				Debugger.LogError("有重复人物id放入助战列表")
	...
	OnSetAssistHeroCallback = function (slot0, slot1)
		if isSuccess(slot0.result) then
			GuildData:SetAssistHero(slot1.hero_id_list)
		else
			ShowTips(GetTips(slot0.result))
		end
	end,
	...
	UpdateGuildPersonRedPoint = function ()
		...
		for slot5, slot6 in ipairs(ClubBossCfg[GuildData:GetBossDifficulty()].personal_reward) do
			if not GuildData:HaveGetGuildBossPersonalReward(slot6[1]) then
				slot1 = slot6[1] <= GuildData:GetBossPersonalScore()
