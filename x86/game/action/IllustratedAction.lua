...
return {
	...
	OnViewIllustration = function (slot0, slot1)
		if isSuccess(slot0.result) then
			IllustratedData:ModifyView(slot1.id, slot1.type)

			if slot1.type == CollectConst.INFORMATION then
				-- Nothing
			elseif slot2 == CollectConst.ENEMY then
				uv0.UpdateEnemyRedPoint()
	...
	UpdateServantRedPoint = function ()
		uv0.UpdateIlluRedPoint(RedPointConst.ILLU_SERVANT, IllustratedData:GetServantInfo(), function (slot0)
			return slot0.is_view == 0
		end)
	end,
	UpdateEquipRedPoint = function ()
		uv0.UpdateIlluRedPoint(RedPointConst.ILLU_EQUIP, IllustratedData:GetEquipInfo(), function (slot0)
			return slot0.is_view == 0
		end)
	end,
	UpdateEnemyRedPoint = function ()
		...
		for slot4, slot5 in pairs(MonsterCfg.get_id_list_by_race) do
			slot6 = false

			for slot10, slot11 in pairs(slot0) do
				if MonsterCfg[slot11.id].race == slot4 and slot11.is_view == 0 then
					slot6 = true
	...
	UpdateWorldViewRedPoint = function ()
		uv0.UpdateIlluRedPoint(RedPointConst.ILLU_WORLDVIEW, IllustratedData:GetAffixInfo(), function (slot0)
			return slot0.is_view == 0
		end)
	end,
	...
	SetIlluRedPoint = function (slot0)
		manager.redPoint:setTip(slot0, 0)
	...
	OnQuerySetBgmBack = function (slot0, slot1)
		if isSuccess(slot0.result) then
			...
			if HomeSceneSettingCfg[HomeSceneSettingData:GetCurScene()].default_music ~= slot1.id then
				slot5 = SettingData:GetHomeSceneSettingData()
