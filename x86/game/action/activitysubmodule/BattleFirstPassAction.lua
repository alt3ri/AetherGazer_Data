...
return {
	Init = function ()
		...
		manager.net:Bind(11057, function (slot0)
	...
	RefreshRedPoint = function (slot0, slot1)
		...
		slot0:RefreshOnceRedPoint(slot1)
	...
	RefreshOnceRedPoint = function (slot0, slot1)
		if BattleFirstPassData:GetBattleFirstPassActivityData(slot1).isNeedRedPoint then
			...
			for slot11, slot12 in ipairs(BattleFirstPassData:GetBattleList(slot1)) do
				if ActivityData:GetActivityData(slot12).startTime <= manager.time:GetServerTime() and BattleFirstPassData:GetBattleFirstPassActivityData(slot1).battleInfo[slot12].battleState == ActivityConst.BATTLE_FIRST_PASS_STATE.UNFINISHED then
	...
	RefreshRewardRedPoint = function (slot0, slot1)
		...
		for slot8, slot9 in ipairs(BattleFirstPassData:GetBattleList(slot1)) do
			if BattleFirstPassData:GetBattleFirstPassActivityData(slot1).battleInfo[slot9].battleState == ActivityConst.BATTLE_FIRST_PASS_STATE.SUCCESS then
