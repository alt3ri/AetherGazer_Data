...
manager.notify:RegistListener(RARE_FISH_CHANGE, function (slot0)
...
return {
	...
	CheckScoreRewardRedPoint = function ()
		...
		for slot6, slot7 in ipairs(ActivityPointRewardCfg.get_id_list_by_activity_id[ActivityConst.SUMMER_FISHING]) do
	...
	Init = function ()
		...
		manager.net:Bind(60001, function (slot0)
			FishingData:Reset()
	...
	OnConstituteFishCallback = function (slot0, slot1)
		if isSuccess(slot0.result) then
			...
			uv0.CheckGroupRewardRedPoint()
			getReward(formatRewardCfgList(ActivitySummerFishGroupCfg[slot1.constitute_id].reward_item_list))
		else
			ShowTips(GetTips(slot0.result))
		end
	end,
