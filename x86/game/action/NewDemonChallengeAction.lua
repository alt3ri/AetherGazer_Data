		...
		if NewDemonChallengeData:GetRewardStateList(slot0) ~= nil then
			...
			for slot8, slot9 in ipairs(NewDemonChallengeRewardCfg.get_id_list_by_activity_id[slot0]) do
				if NewDemonChallengeRewardCfg[slot9] ~= nil then
					if slot1[slot9] == ActivityConst.DEMON_CHALLENGE_REWARD_STATE.UNRECEIVE and slot10.complete_num <= (NewDemonChallengeData:GetCompletedDifficultyCountList(slot0)[slot10.difficulty_id] or 0) then
						slot4 = true
