...
function slot0.InitPlayer(slot0, slot1)
	if not slot0.timer_ then
		slot0.timer_ = FuncTimerManager.inst:CreateFuncFrameTimer(function ()
			if uv0.player_ then
...
function slot0.StartStory(slot0, slot1)
	slot2 = manager.ui:GetUISeparateRender()

	if StoryCfg[slot1].trigger[4] == 1 then
		gameContext:Go("/blank")
		manager.audio:Pause("music", true)
		manager.video:Play(slot3.videoPath, function (slot0)
			slot2 = nil
	...
	slot5, slot6 = slot0:SetCameraParam()
	...
	function slot0.player_.onStoryFinished_(slot0)
		...
		PlayGameSetBGM()
		manager.ui:SetUISeparateRender(uv4)

		if uv5.videoPath and uv5.videoPath ~= "" then
			gameContext:Go("/blank")
			manager.audio:PauseAll(true)
			manager.video:Play(uv5.videoPath, function ()
				gameContext:Back()
				PlayerAction.ChangeStoryList(uv0)
...
function slot0.StartStoryById(slot0, slot1, slot2, slot3)
	if StoryCfg[slot1].trigger[4] == 1 then
		manager.audio:Pause("music", true)
		manager.video:Play(slot4.videoPath, function (slot0)
			slot2 = nil
	...
	slot6, slot7 = slot0:SetCameraParam()
	...
	for slot10, slot11 in pairs(StoryCfg) do
		if slot11.trigger[1] == uv0 and slot12[2] == slot1 and slot12[3] == slot2 then
			...
			if slot12[4] == 1 then
				...
				manager.video:Play(slot11.videoPath, function (slot0)
					slot2 = nil
...
function slot0.CheckBeforeBattleStory(slot0, slot1, slot2, slot3)
	...
	for slot10, slot11 in pairs(StoryCfg) do
		if slot11.trigger[1] == uv0 and slot12[2] == slot1 and slot12[3] == slot4 then
			slot6 = slot10
...
function slot0.SetChessBG(slot0, slot1)
	WaitRenderFrameUtil.inst.StartScreenShot(function ()
		...
		if uv1.player_.bgs_.ChessBG == nil then
			...
			slot4:GetComponent("SpriteRenderer").sprite = slot0:GetSprite()
			slot4.name = slot3
...
function slot0.SetStagePos(slot0)
	slot0.stage_.transform.position = Vector3(0, -500, 0)
...
function slot0.SetCameraParam(slot0, slot1)
	...
	if (slot1 or 0) == 0 then
		manager.ui.mainCameraCom_.fieldOfView = Mathf.Lerp(25, 30, math.abs((manager.ui.mainCameraCom_.pixelWidth / manager.ui.mainCameraCom_.pixelHeight - 2.335766423357664) / -1.0024330900243308))
	else
	...
	slot0.delay_timer_ = Timer.New(function ()
		if manager.ui.mainCamera:GetComponent("CameraExtension") then
			...
			slot0.radialWarpLerp = 0
			slot0.radialWarpScale = 0
			slot0.radialWarpDensity = 0
			slot0.radialAbberationIntensity = 0
		end
	end, 0.05, false, true)
...
function slot0.ResetCameraParam(slot0)
	...
	if manager.ui.mainCamera:GetComponent("CameraExtension") then
		slot4.shadowRotationImmediately = slot0.shadowRotationImmediately_
		slot4.radialBlurScale = slot0.radialBlurScal_
	end
