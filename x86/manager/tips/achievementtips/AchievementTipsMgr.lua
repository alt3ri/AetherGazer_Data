...
function slot0.AddAchievementID(slot0, slot1)
	if #slot0.fliterTypeList_ > 0 and not table.keyof(slot0.fliterTypeList_, AchievementCfg[slot1].type_id) then
...
function slot0.InitUI(slot0)
	slot0.gameObject_ = GameObject.Instantiate(Asset.Load("Widget/Common/Pop/AchievementTipsUI"), GameObject.Find("UICamera_DontDestroy/Canvas/UIMessage").transform)
	slot0.transform_ = slot0.gameObject_.transform

	ComponentBinder.GetInstance():BindCfgUI(slot0, slot0.gameObject_)

	slot0.typeController_ = slot0.controllerEx_:GetController("icon")
end
