...
function slot0.GetGameServerWeek(slot0)
	return math.ceil(((slot0:GetServerTime() - slot0._sAnchorTime - GameSetting.refresh_time1.value[1][1] * 3600) % uv0 + 1) / uv1)
end
...
function slot0.DescCDTimeEndMinute(slot0, slot1)
	slot2 = math.floor(slot1 / 3600)

	return string.format("%02d:%02d", slot2, math.floor((slot1 - slot2 * 3600) / 60))
end
...
function slot0.GetLostTimeStrWith2Unit(slot0, slot1, slot2)
	if not slot1 then
		return
	end

	slot3 = slot1 - slot0:GetServerTime()
	slot6 = math.fmod(math.floor(slot3 / 60), 60)
	slot7 = math.fmod(slot3, 60)
	slot8 = ""
	slot11 = GetTips("MINUTE")
	slot12 = GetTips("SECOND")

	if math.floor(slot3 / 86400) > 0 then
		slot8 = slot4 .. GetTips("DAY") .. math.fmod(math.floor(slot3 / 3600), 24) .. GetTips("HOUR")
	elseif slot5 > 0 then
		slot8 = slot5 .. slot10 .. slot6 .. slot11
	elseif slot6 > 0 then
		slot8 = 0 .. slot10 .. slot6 .. slot11
	elseif slot7 > 0 then
		slot8 = 0 .. slot10 .. 1 .. slot11
	elseif slot2 then
		slot8 = 0 .. slot10 .. 1 .. slot11
	else
		return GetTips("TIME_OUT")
...
function slot0.GetMonth(slot0, slot1, slot2)
	return tonumber(slot0:STimeDescS(slot1 or slot0:GetServerTime(), slot2 or "!%m"))
end
...
function slot0.FormatChatTime(slot0, slot1)
	if tonumber(os.date("!%Y", slot1)) ~= tonumber(os.date("!%Y", slot0:GetServerTime())) then
		return os.date("%Y/%m/%d", slot1)
	elseif not slot0:IsSameDay(slot1, slot3) then
		return os.date("%m/%d", slot1)
	else
		return os.date("%H:%M", slot1)
	end
end
