...
function slot0.Ctor(slot0)
	...
	slot0.isCheckMemoryLeak_ = true
end

function slot0.CacheReduxView(slot0, slot1)
	if true and slot1:Cacheable() and not table.indexof(slot0.cachedReduxViewList_, slot1) then
...
function slot0.RemoveView(slot0, slot1)
	if table.indexof(slot0.cachedReduxViewList_, slot1) then
		...
		if table.indexof(slot0.reduxViewList_, slot1) then
			table.remove(slot0.reduxViewList_, slot3)
		end
...
function slot0.GetReduxView(slot0, slot1, slot2)
	...
	if slot0.reduxViewDic_[slot1] ~= nil then
		...
		slot3 = slot2.component.New()

		slot3:SetRouteName(slot1)
		table.insert(slot0.reduxViewList_, slot3)
		slot0:PrintViewList()
	end
...
function slot0.PrintExistingObjectInfo(slot0)
	...
	if slot0.existReduxViewList_ ~= nil and #slot0.existReduxViewList_ > 0 then
		...
		for slot6 = 1, #slot0.existReduxViewList_ do
			slot2 = "<color=" .. "#ff0000" .. ">UI系统结束时，未被Dispose的ReduxView数量：" .. #slot0.existReduxViewList_ .. slot6 .. slot0.existReduxViewList_[slot6].class.__cname .. (slot6 == #slot0.existReduxViewList_ and "" or ",")
		end

		CustomLog.LogError(slot2 .. "请优先按规范销毁对象</color>")
	else
