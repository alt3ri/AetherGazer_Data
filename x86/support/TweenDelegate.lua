...
function slot0.PlayScale(slot0, slot1, slot2)
	if slot1.isInitFrom == 1 then
		LuaHelper.SetTfLoalScaleValue(slot0.transform_:Find(slot1.go).gameObject.transform, slot1.fromData.x, slot1.fromData.y, slot1.fromData.z)
	end

	if slot2 and slot2.onComplete then
		LeanTween.scale(slot3, Vector3(slot1.toData.x, slot1.toData.y, slot1.toData.z), slot1.time):setEase(LeanTweenType.IntToEnum(slot1.easeType)):setDelay(slot1.delay):setOnComplete(LuaHelper.VoidAction(function ()
			uv0.onComplete()
		end))
	end
end
...
function slot0.PlayMove(slot0, slot1, slot2)
	...
	if slot2 and slot2.onComplete then
		LeanTween.moveLocal(slot3, uv0.GetAdaptedLocalPosition(slot3, slot1.toData), slot1.time):setEase(LeanTweenType.IntToEnum(slot1.easeType)):setDelay(slot1.delay):setOnComplete(LuaHelper.VoidAction(function ()
			uv0.onComplete()
		end))
	end
end
...
function slot0.PlayAlpha(slot0, slot1, slot2)
	...
	if slot1.isInitFrom == 1 then
		LeanTween.alphaCanvas(uv0.GetOrAddComponent((type(slot1.go) ~= "string" or slot0.transform_:Find(slot1.go).gameObject) and slot1.go, typeof(CanvasGroup)), slot1.fromData.x, 0)
	end

	if slot2 and slot2.onComplete then
		LeanTween.alphaCanvas(slot4, slot1.toData.x, slot1.time):setEase(LeanTweenType.IntToEnum(slot1.easeType)):setDelay(slot1.delay):setOnComplete(LuaHelper.VoidAction(function ()
			uv0.onComplete()
		end))
	end
end
...
function slot0.PlayHide(slot0, slot1, slot2)
	...
	if slot2 and slot2.onComplete then
		LeanTween.delayedCall(slot1.delay + slot1.time, LuaHelper.VoidAction(function ()
			SetActive(uv0, true)
	...
	for slot9 = 1, slot0.transform_:Find(slot1.go).childCount do
		if slot4:GetChild(slot3).gameObject:GetComponent("TweenManager") ~= nil then
			if slot1.fromData.x == 0 then
				TweenDelegate.New(slot12, slot10):Play("ItemShow", nil, slot1.delay + slot3 * slot1.toData.x)
			else
...
function slot0.PlayRotate(slot0, slot1, slot2)
	if slot1.isInitFrom == 1 then
		LuaHelper.SetTfLocalRotateValue(slot0.transform_:Find(slot1.go).gameObject.transform, slot1.fromData.x, slot1.fromData.y, slot1.fromData.z)
	end

	if slot2 and slot2.onComplete then
		LeanTween.rotateLocal(slot3, Vector3(slot1.toData.x, slot1.toData.y, slot1.toData.z), slot1.time):setEase(LeanTweenType.IntToEnum(slot1.easeType)):setDelay(slot1.delay):setOnComplete(LuaHelper.VoidAction(function ()
			uv0.onComplete()
		end))
	end
end
